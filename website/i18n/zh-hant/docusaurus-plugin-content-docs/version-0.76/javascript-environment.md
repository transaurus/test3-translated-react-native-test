...
- 若停用 Hermes，React Native 會改用 Safari 的 JavaScript 引擎 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)。注意在 iOS 上，由於應用程式缺乏可寫入的執行記憶體，JavaScriptCore 無法使用 JIT 編譯
- 使用 Chrome 偵錯時，所有 JavaScript 程式碼會在 Chrome 內部執行，並透過 WebSockets 與原生程式碼通訊。Chrome 使用 [V8](https://v8.dev/) 作為 JavaScript 引擎
