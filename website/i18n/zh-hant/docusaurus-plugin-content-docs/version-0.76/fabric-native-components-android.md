...
現在是時候編寫一些 Android 平台代碼來渲染網頁視圖。您需要遵循的步驟如下：
...
Codegen 將生成您需要實現的 `ViewManager` 接口以及網頁視圖的 `ViewManager` 委託。
...
該類定義了三個 Android 構造函數，但它們的實際實現委託給了私有的 `configureComponent` 函數。此函數負責初始化所有組件特定的屬性：在此代碼中，您設置了 `WebView` 的佈局，並定義了用於自定義 `WebView` 行為的 `WebClient`。在此代碼中，`ReactWebView` 在頁面加載完成時通過實現 `WebClient` 的 `onPageFinished` 方法來發出事件。
...
然後，視圖管理器需要定義所有 React 組件屬性如何更新原生視圖。在示例中，您需要決定如何處理 React 將在 `WebView` 上設置的 `sourceURL` 屬性。
