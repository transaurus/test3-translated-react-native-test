...
- **若沒有監聽器返回 true 或未註冊任何監聽器**，系統將預設執行返回按鈕功能來退出應用程式。

> **模態視窗使用者請注意：** 若應用程式正在顯示開啟的 `Modal`，`BackHandler` 將不會發布任何事件（參見 [`Modal` 文件](modal#onrequestclose)）。
...
[React Native Hooks](https://github.com/react-native-community/hooks#usebackhandler) 提供了一個簡便的 `useBackHandler` 鉤子，可簡化事件監聽器的設定流程。
