...
React Native 提供了許多內建的[核心組件](intro-react-native-components)，可直接用於您的應用程式。您可以在左側邊欄（或螢幕較窄時的上方選單）中找到所有組件。如果不確定從何開始，請參考以下分類：
...
這些組件可能對特定應用程式有用。如需完整的組件和 API 列表，請查看左側邊欄（或螢幕較窄時的上方選單）。
