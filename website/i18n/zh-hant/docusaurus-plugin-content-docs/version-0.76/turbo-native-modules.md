...
本指南將展示如何實作 [Web Storage API](https://html.spec.whatwg.org/multipage/webstorage.html#dom-localstorage-dev)：`localStorage`。該 API 對於可能在您的專案中撰寫應用程式程式碼的 React 開發者來說是熟悉的。

為了在移動設備上實現此功能，我們需要使用 Android 和 iOS 的 API：
...
React Native 提供了一個名為 [Codegen](/the-new-architecture/what-is-codegen.md) 的工具，該工具接受以 TypeScript 或 Flow 編寫的規格，並為 Android 和 iOS 生成平台特定的程式碼。規格宣告了將在您的原生程式碼和 React Native JavaScript 運行時之間傳遞的方法和資料類型。Turbo 原生模組既是您的規格，也是您編寫的原生程式碼，以及從您的規格生成的 Codegen 介面。

創建規格檔案的步驟：

1. 在您的應用程式根目錄中，創建一個名為 `specs` 的新資料夾。
...
使用 `NativeLocalStorage`，以下是修改後的 `App.tsx`，其中包含一些我們希望持久化的文本、一個輸入欄位和一些按鈕來更新此值。
