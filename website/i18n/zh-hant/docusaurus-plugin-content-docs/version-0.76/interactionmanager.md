...
`runAfterInteractions` 接受一個普通回調函數，或是一個帶有返回 `Promise` 的 `gen` 方法的 `PromiseTask` 對象。如果提供了 `PromiseTask`，則會完全解析（包括那些也通過 `runAfterInteractions` 安排更多任務的異步依賴項），然後才開始處理可能之前同步排隊的下一個任務。
...
正數將使用 setTimeout 在事件循環運行時間達到截止值後安排任何任務，否則所有任務會在一個 setImmediate 批次中執行（默認）。