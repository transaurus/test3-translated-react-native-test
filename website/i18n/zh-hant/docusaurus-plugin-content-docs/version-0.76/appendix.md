...
- **Spec** - 用於描述 Turbo 原生模組或 Fabric 原生元件 API 的 TypeScript 或 Flow 程式碼。由 **Codegen** 用於生成樣板程式碼。

- **原生模組** - 不包含使用者介面(UI)的原生函式庫。例如持久儲存、通知、網路事件等。這些可透過函式與物件讓您的 JavaScript 應用程式程式碼存取。
- **原生元件** - 可透過 React 元件讓您的應用程式 JavaScript 程式碼使用的原生平台視圖。

- **舊版原生元件** - 運行在舊版 React Native 架構上的元件。
- **舊版原生模組** - 運行在舊版 React Native 架構上的模組。
...
<b>[1]</b> 我們強烈建議使用物件字面值(Object literals)而非物件(Objects)。

:::info
您可能也會發現參考 React Native 核心模組的 JavaScript 規格很有幫助。這些規格位於 React Native 儲存庫中的 `Libraries/` 目錄內。
:::