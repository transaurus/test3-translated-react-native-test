...
Keyboard 模組允許您監聽原生事件並對其作出反應，同時也能對鍵盤進行操作，例如關閉鍵盤。
...
> 注意：在 Android 上僅支援 `keyboardDidShow` 和 `keyboardDidHide` 事件。若您的 activity 將 `android:windowSoftInputMode` 設為 `adjustNothing`，則在 Android 10 及以下版本中這些事件不會被觸發。
