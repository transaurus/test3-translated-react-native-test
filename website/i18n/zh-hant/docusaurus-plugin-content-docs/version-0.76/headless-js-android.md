...
你可以在任務中執行任何操作，例如網絡請求、計時器等，只要不涉及 UI。一旦任務完成（即 Promise 被解析），React Native 將進入「暫停」模式（除非有其他任務正在運行，或有前台應用程式）。
...
- 預設情況下，如果嘗試在前台運行任務，應用程式會崩潰。這是為了防止開發者因在任務中執行大量工作而拖慢 UI。你可以傳遞第四個 `boolean` 參數來控制此行為。
...
廣播接收器然後在 `onReceive` 函數中處理廣播的 intent。這是檢查應用程式是否處於前台的好地方。如果應用程式不在前台，我們可以準備啟動 intent，無需信息或使用 `putExtra` 捆綁額外信息（請記住 bundle 只能處理可序列化的值）。最後啟動服務並獲取 wakelock。
