...
接著我們需要建立 `NativeLocalStoragePackage`。它通過將模組封裝為 Turbo Native Package，提供了一個在 React Native 運行時註冊模組的物件：
...
最後，我們需要告訴主應用程式中的 React Native 如何找到這個 `Package`。這在 React Native 中稱為「註冊」套件。
...
:::info
後續您將學習如何將 Turbo Native 模組作為 [npm 套件](the-new-architecture/create-module-library.md#publish-the-library-on-npm) 發佈，我們的建置工具會自動為您進行連結。
:::
