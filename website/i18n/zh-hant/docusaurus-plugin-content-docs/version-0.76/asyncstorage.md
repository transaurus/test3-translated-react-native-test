...
建議在輕量使用之外，應基於 `AsyncStorage` 進行封裝而非直接調用，因其操作為全域性質。

在 iOS 上，`AsyncStorage` 底層使用原生代碼將小值存儲於序列化字典，大值存於獨立文件。Android 端則會根據可用性選擇使用 [RocksDB](https://rocksdb.org/) 或 SQLite。

JavaScript 層的 `AsyncStorage` 提供清晰 API、真實 `Error` 物件及非批量方法，每個方法均返回 `Promise` 物件。
...
合併現有 `key` 值與輸入值（假設均為 JSON 字串）。返回 `Promise` 物件。
...
清除 _所有_ 客戶端、函式庫等的 `AsyncStorage` 資料。通常應改用 `removeItem` 或 `multiRemove` 僅清除應用自有鍵值。返回 `Promise` 物件。
...
獲取應用已知的 _所有_ 鍵值（含其他調用者及函式庫）。返回 `Promise` 物件。
...
此方法允許您批次獲取給定鍵名陣列對應的項目。回調函數將被調用並傳入找到的鍵值對陣列：
