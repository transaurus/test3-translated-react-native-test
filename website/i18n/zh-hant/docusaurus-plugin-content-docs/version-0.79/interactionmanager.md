...
觸控處理系統將一或多個有效觸控視為「互動」，並會延遲 `runAfterInteractions()` 回調直到所有觸控結束或被取消。

InteractionManager 還允許應用程式在動畫開始時創建互動「句柄」來註冊動畫，並在完成時清除：
...
`runAfterInteractions` 接受普通回調函數或帶有返回 `Promise` 的 `gen` 方法的 `PromiseTask` 物件。若提供 `PromiseTask`，則會完全解析（包括透過 `runAfterInteractions` 安排更多任務的非同步依賴項）後才開始執行可能先前同步排隊的下一個任務。

預設情況下，排隊任務會在一個 `setImmediate` 批次中一起循環執行。若使用正數調用 `setDeadline`，則任務僅會執行到接近截止時間（以 js 事件循環運行時間為準），此時會透過 setTimeout 讓出執行權，允許觸控等事件開始互動並阻止排隊任務執行，從而提高應用程式響應性。
...
安排一個函數在所有互動完成後執行。返回可取消的「promise」。
...
正數值會使用 setTimeout 在事件循環運行時間達到截止值後安排任務，否則所有任務會在一個 setImmediate 批次中執行（預設）。