...
可以同時掛載多個`StatusBar`元件。這些元件的屬性會按照掛載順序合併。
...
對於不適合使用元件的情況，該元件也提供了以靜態函數形式公開的命令式API。但不建議同時使用靜態API和元件來設定相同的屬性，因為靜態API設定的值會在下次渲染時被元件設定的值覆蓋。
...
狀態列屬性變更時是否應顯示過渡動畫。支援`backgroundColor`、`barStyle`和`hidden`屬性。
...
:::warning
由於Android 15引入了邊到邊強制執行，在API級別35中設定狀態列背景顏色已被棄用，設定後不會產生任何效果。您可以在此處閱讀更多關於[邊到邊建議](https://github.com/react-native-community/discussions-and-proposals/discussions/827)。
:::
...
狀態列是否為半透明。當設為`true`時，應用程式會在狀態列下方繪製。這在使用半透明狀態列顏色時非常有用。

:::warning
由於Android 15引入了邊到邊強制執行，在API級別35中設定狀態列為半透明已被棄用，設定後不會產生任何效果。您可以在此處閱讀更多關於[邊到邊建議](https://github.com/react-native-community/discussions-and-proposals/discussions/827)。
:::
