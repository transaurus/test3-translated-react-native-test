...
- 各類 `shadow` 屬性（`shadowColor`、`shadowOffset`、`shadowOpacity`、`shadowRadius`）：這些直接映射到底層平台 API 暴露的原生對應功能
...
- `dropShadow` 僅限 Android 平台，`boxShadow` 支援 iOS 和 Android
- `dropShadow` 無法像 `boxShadow` 那樣設置內陰影
- `dropShadow` 沒有 `boxShadow` 的 `spreadDistance` 參數

總體而言，`boxShadow` 和 `dropShadow` 的功能比 `shadow` 屬性更強大。但 `shadow` 屬性直接映射到原生平台 API，若只需基礎陰影效果建議優先使用。注意僅 `shadowColor` 同時支援 Android 和 iOS，其他 `shadow` 屬性僅在 iOS 平台生效。

## 屬性

### `boxShadow`

詳見 [視圖樣式屬性](./view-style-props#boxshadow) 文件說明。

### `dropShadow` <div class="label android">Android</div>

詳見 [視圖樣式屬性](./view-style-props#filter) 文件說明。
