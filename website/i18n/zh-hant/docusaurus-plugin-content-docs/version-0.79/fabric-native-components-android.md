...
### 1. 通過 Gradle 運行 Codegen

執行一次此操作以生成您的 IDE 可以使用的樣板代碼。
...
Codegen 將生成您需要實現的 `ViewManager` 接口以及網頁視圖的 `ViewManager` 代理。

### 2. 編寫 `ReactWebView`

`ReactWebView` 是封裝 Android 原生視圖的組件，當 React Native 使用我們的自定義組件時會渲染該視圖。

在 `android/src/main/java/com/webview` 文件夾中創建一個 `ReactWebView.java` 或 `ReactWebView.kt` 文件，並添加以下代碼：
...
該類定義了三個 Android 構造函數，但它們的實際實現委託給了私有的 `configureComponent` 函數。此函數負責初始化所有特定於組件的屬性：在此情況下，您設置了 `WebView` 的佈局，並定義了用於自定義 `WebView` 行為的 `WebClient`。在此代碼中，`ReactWebView` 在頁面加載完成時通過實現 `WebClient` 的 `onPageFinished` 方法發出事件。
...
文件的最後部分包含您發送事件所需的數據類型的定義：

- `OnScriptLoadedEventResult`，包含 `OnScriptLoaded` 事件的可能結果；
- 實際的 `OnScriptLoadedEvent`，它需要繼承 React Native 的 `Event` 類。

### 3. 編寫 `WebViewManager`

`WebViewManager` 是將 React Native 運行時與原生視圖連接起來的類。
...
它持有 `CustomWebViewManagerDelegate` 的引用，這是 Codegen 生成的另一個元素。

然後它覆蓋了 `getName` 函數，該函數必須返回與 spec 的 `codegenNativeComponent` 函數調用中使用的名稱相同。

`createViewInstance` 函數負責實例化一個新的 `ReactWebView`。

接著，ViewManager 需要定義所有 React 組件屬性如何更新原生視圖。在示例中，您需要決定如何處理 React 將在 `WebView` 上設置的 `sourceURL` 屬性。
