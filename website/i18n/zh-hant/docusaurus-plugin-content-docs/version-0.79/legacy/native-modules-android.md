...
建議使用 Android Studio 編寫原生程式碼。作為專為 Android 開發打造的 IDE，它能快速協助解決程式碼語法等輕微問題。
...
> 但建議採用如上所示的 `ReactContextBaseJavaModule`，它提供了 `ReactApplicationContext` (RAC) 的存取權限，對於需要掛載活動生命週期方法的原生模組非常有用。未來此做法也將更容易實現類型安全的原生模組。React Native 會根據每個原生模組的 JavaScript 規範生成抽象基礎類別，該類別即擴展自 `ReactContextBaseJavaModule`。
...
接著您需在原生模組中添加一個能建立日曆事件並供 JavaScript 呼叫的方法。所有需從 JavaScript 觸發的原生模組方法都必須標記 `@ReactMethod` 註解。

為 `CalendarModule` 設定 `createCalendarEvent()` 方法，未來可透過 `CalendarModule.createCalendarEvent()` 在 JS 端呼叫。目前該方法將接收名稱和位置兩個字串參數，參數類型選項將稍後說明。
...
在方法內添加除錯日誌，以確認從應用程式呼叫時確實觸發。以下是從 Android util 套件導入並使用 [Log](https://developer.android.com/reference/android/util/Log) 類別的範例：
...
React Native 調用 `ReactPackage` 上的 `createNativeModules()` 方法以獲取要註冊的原生模組列表。對於 Android，如果模組未在 createNativeModules 中實例化並返回，則無法從 JavaScript 中使用。
...
至此，您已為 Android 的原生模組建立了基本架構。請透過在 JavaScript 中存取原生模組並呼叫其導出方法來測試功能。

在應用程式中找到適合呼叫原生模組 `createCalendarEvent()` 方法的位置。以下是一個可新增至應用程式的元件範例 `NewModuleButton`，您可以在該按鈕的 `onPress()` 函數中觸發原生模組呼叫。
...
現在您已可呼叫 `CalendarModule` 的 `createCalendarEvent()` 原生方法。以下示範如何在 `NewModuleButton` 的 `onPress()` 方法中實作：
...
最後一步是重新建置 React Native 應用程式，使最新原生程式碼（包含您的新原生模組）生效。在 React Native 專案根目錄執行以下指令：
...
當您依照指南迭代開發原生模組時，需重新建置原生應用程式才能讓 JavaScript 端取得最新變更。這是因為您編寫的程式碼屬於應用程式的原生部分。雖然 React Native 的 Metro 打包工具會監控 JavaScript 變更並即時重新建置，但不會對原生程式碼執行相同操作。因此若要測試最新的原生修改，必須使用上述指令重新建置。

### 重點回顧 ✨

現在您應能在應用程式中呼叫原生模組的 `createCalendarEvent()` 方法。在我們的範例中，透過點擊 `NewModuleButton` 觸發。您可透過檢視 `createCalendarEvent()` 方法中設定的日誌來確認功能。請依照[這些步驟](https://developer.android.com/studio/debug/am-logcat.html)查看應用程式的 ADB 日誌。搜尋您設定的 `Log.d` 訊息（範例中為「Create event called with name: testName and location: testLocation」），每次呼叫原生模組方法時都應能看到該日誌訊息。
...
至此您已成功建立 Android 原生模組，並在 React Native 應用程式中從 JavaScript 呼叫其原生方法。後續可進一步學習原生模組方法的參數類型設定，以及如何配置回調函數與 Promise。

## 擴展日曆原生模組功能

### 優化原生模組導出方式

目前透過 `NativeModules` 提取原生模組的方式較為繁瑣。

為提升使用便利性，可為模組建立 JavaScript 封裝層。建立名為 `CalendarModule.js` 的新 JavaScript 檔案，內容如下：
...
此 JavaScript 檔案也是添加 JavaScript 端功能的理想位置。例如若使用 TypeScript 等類型系統，可在此為原生模組添加類型註解。雖然 React Native 尚未支援原生到 JS 的類型安全，但所有 JS 程式碼都將具備類型安全。此做法也能為未來遷移到類型安全的原生模組預做準備。以下為替 CalendarModule 添加類型安全的範例：
...
對於未列出的參數類型，您需自行處理轉換。例如在 Android 中，`Date` 類型不支援開箱即用的轉換。您可像這樣在原生方法中自行處理 `Date` 類型的轉換：
...
技術上可直接從原生模組物件存取 `getConstants()` 導出的常數。但 TurboModules 將不再支援此做法，因此我們建議社群改用上述方式，避免未來不必要的遷移。
...
您可在 Java/Kotlin 方法中呼叫回調，傳遞任何想送至 JavaScript 的數據。請注意只能從原生代碼傳遞可序列化數據至 JavaScript。若需回傳原生物件，可使用 `WriteableMaps`；若需使用集合，則用 `WritableArrays`。另需強調的是，回調不會在原生函數完成後立即觸發。以下範例將先前呼叫中建立的事件 ID 傳遞給回調：
...
另一重要細節是：原生模組方法只能呼叫一次回調。這意味著您只能呼叫成功回調或失敗回調其一，且每個回調最多只能觸發一次。不過原生模組可儲存回調並延後觸發。
...
更多細節，您可以在此處找到 `Promise.java` 接口：[這裡](https://github.com/facebook/react-native/blob/main/packages/react-native/ReactAndroid/src/main/java/com/facebook/react/bridge/Promise.java)。如果未提供 `userInfo`，React Native 會將其設為 null。對於其餘參數，React Native 會使用預設值。`message` 參數提供了錯誤 `message`，顯示在錯誤調用堆疊的頂部。以下是從 Java/Kotlin 中的 reject 調用顯示在 JavaScript 中的錯誤訊息範例。

Java/Kotlin 的 reject 調用：
...
原生模組可以在不被直接調用的情況下向 JavaScript 發送事件。例如，您可能希望向 JavaScript 發出提醒，表示來自原生 Android 日曆應用程式的日曆事件即將發生。最簡單的方法是使用 `RCTDeviceEventEmitter`，可以從 `ReactContext` 中獲取，如下面的程式碼片段所示。
...
讓我們實現一個基本的圖片選擇器來演示這一點。圖片選擇器將向 JavaScript 公開 `pickImage` 方法，該方法在被調用時會返回圖片的路徑。
...
監聽活動的生命週期事件（如 `onResume`、`onPause` 等）與實現 `ActivityEventListener` 的方式非常相似。模組必須實現 `LifecycleEventListener`，然後在模組的構造函數中註冊監聽器，如下所示：
...
截至目前，在 Android 上，所有原生模組的異步方法都在單一線程上執行。原生模組不應對其被調用的線程有任何假設，因為當前的分配方式可能在未來發生變化。如果需要進行阻塞調用，應將繁重的工作分派到內部管理的工作線程，並從那裡分發任何回調。