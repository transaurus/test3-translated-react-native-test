...
[`flexDirection`](layout-props#flexdirection) 控制子節點的排列方向（也稱為主軸）。交叉軸則是與主軸垂直的軸線，或是換行排列的軸線。
...
[`justifyContent`](layout-props#justifycontent) 描述如何在容器的主軸上對齊子元素。例如，你可以使用此屬性在 `flexDirection` 設為 `row` 的容器中水平置中子元素，或在 `flexDirection` 設為 `column` 的容器中垂直置中子元素。

- `flex-start`（**預設值**）將容器的子元素對齊至主軸起始端。

- `flex-end` 將容器的子元素對齊至主軸末端。

- `center` 將容器的子元素對齊至主軸中心。

- `space-between` 沿主軸均勻分布子元素，剩餘空間分配於子元素之間。

- `space-around` 沿主軸均勻分布子元素，剩餘空間環繞子元素分配。相較於 `space-between`，使用 `space-around` 會在第一個子元素前和最後一個子元素後也分配空間。

- `space-evenly` 在主軸上完全均勻分布子元素。相鄰項目間距、主軸起始端與首個項目間距、主軸末端與末個項目間距均完全相等。

詳細說明請參閱[此處](https://www.yogalayout.dev/docs/styling/justify-content)。
...
[`alignItems`](layout-props#alignitems) 定義子元素如何沿容器的交叉軸對齊。其功能與 `justifyContent` 相似，但作用於交叉軸而非主軸。

- `stretch`（**預設值**）拉伸子元素以匹配容器交叉軸的高度。
...
:::info
要使 `stretch` 生效，子元素在次軸上不能有固定尺寸。下例中，除非移除子元素的 `width: 50` 設定，否則 `alignItems: stretch` 不會產生效果。
:::

詳細說明請參閱[此處](https://www.yogalayout.dev/docs/styling/align-items-self)。
...
[`alignSelf`](layout-props#alignself) 的選項與效果同 `alignItems`，但作用於單一子元素，可覆寫父容器透過 `alignItems` 設定的對齊方式。
...
- `stretch`（網頁版 Yoga 的預設值）拉伸換行內容以匹配容器交叉軸高度。

- `center` 將換行內容對齊至交叉軸中心。

- `space-between` 沿交叉軸均勻分布換行內容，剩餘空間分配於行間。

- `space-around` 沿交叉軸均勻分布換行內容，剩餘空間環繞各行分配。容器兩端的空間為行間距的一半。

- `space-evenly` 完全均勻分布換行內容，所有間距（包含容器兩端）大小相同。

詳細說明請參閱[此處](https://www.yogalayout.dev/docs/styling/align-content)。
...
[`flexWrap`](layout-props#flexwrap) 屬性設置在容器上，用於控制當子元素沿主軸溢出容器尺寸時的處理方式。預設情況下，子元素會被強制排列在單一行內（這可能會壓縮元素）。如果允許換行，則會根據需要將項目沿主軸換行成多行。
...
- [`flexBasis`](layout-props#flexbasis) 是一種與軸向無關的方式，用於設定項目在主軸上的預設大小。設置子元素的 `flexBasis` 類似於在父容器為 `flexDirection: row` 時設定該子元素的 `width`，或在父容器為 `flexDirection: column` 時設定其 `height`。項目的 `flexBasis` 是該項目的預設大小，即在進行任何 `flexGrow` 和 `flexShrink` 計算之前的大小。
...
- [`flexShrink`](layout-props#flexshrink) 描述當子元素的總大小在主軸上超出容器尺寸時，如何沿主軸縮小子元素。`flexShrink` 與 `flexGrow` 非常相似，可以將任何溢出空間視為負的剩餘空間來理解。這兩個屬性也能很好地協同工作，允許子元素根據需要伸縮。

  `flexShrink` 接受任何大於或等於 0 的浮點數，預設值為 0（在網頁上預設為 1）。容器會根據子元素的 `flexShrink` 值加權縮小子元素。
...
- `absolute` 當元素採用絕對定位時，不會參與正常的佈局流，而是獨立於其兄弟元素進行佈局。其位置由 `top`、`right`、`bottom` 和 `left` 的值決定，這些值會將元素相對於其包含塊（containing block）進行定位。

- `static` 當元素採用靜態定位時，會依照正常的佈局流進行定位，並忽略 `top`、`right`、`bottom` 和 `left` 的值。此 `position` 設定也會導致元素不會為其絕對定位的子元素形成包含塊，除非存在其他優先的樣式屬性（例如 `transform`）。這允許 `absolute` 元素可以定位到非其父元素的物件上。注意：**`static` 僅在新架構（New Architecture）中可用**。
...
元素的包含塊是控制其位置和大小的祖先元素。React Native 中的包含塊運作方式與 [網頁上的運作方式](https://developer.mozilla.org/en-US/docs/Web/CSS/Containing_block) 非常相似，但由於缺少某些網頁功能而有所簡化。

絕對定位元素的 `top`、`right`、`bottom` 和 `left` 值會相對於其包含塊進行計算。

套用於絕對定位元素的百分比長度（例如：`width: '50%'` 或 `padding: '10%'`）會相對於其包含塊的大小進行計算。例如，如果包含塊的寬度為 100 點，則絕對定位元素上的 `width: 50%` 會使其寬度為 50 點。
