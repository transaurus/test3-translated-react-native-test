...
決定觸控激活時被包裹視圖的不透明度值。取值範圍應在 0 到 1 之間，預設為 0.85。需設置 `underlayColor` 方可生效。
...
_(僅限 Apple TV)_ 電視首選聚焦狀態（參見 View 元件的文件說明）。
