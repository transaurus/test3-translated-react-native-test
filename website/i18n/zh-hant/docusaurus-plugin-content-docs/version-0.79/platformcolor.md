...
您可以使用 `PlatformColor` 函數，透過傳入對應的原生顏色字串值來存取目標平台上的原生顏色。將字串傳遞給 `PlatformColor` 函數後，若該平台存在此顏色，函數將返回對應的原生顏色，您可將其應用於應用程式的任何部分。
...
由於原生顏色可能受主題或高對比度設定影響，此平台特定邏輯也會在您的元件內部自動轉換。
...
傳遞給 `PlatformColor` 函數的字串值必須完全匹配應用程式運行時所在原生平台的對應字串。為避免運行時錯誤，應透過 `Platform.OS === '平台名稱'` 或 `Platform.select()` 進行平台檢查（如上例所示）來包裝此函數。
