...
> 對於 React 元件來說，[`useWindowDimensions`](usewindowdimensions) 是更推薦的 API。與 `Dimensions` 不同，它會隨著視窗尺寸的變化而更新。這與 React 的設計模式非常契合。
...
> 在 Android 上，`window` 的尺寸將排除被 `狀態欄`（如果不透明）和 `底部導航欄` 使用的空間。
