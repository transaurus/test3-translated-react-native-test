...
現在是時候撰寫一些 Android 平台代碼來渲染網頁視圖了。您需要遵循的步驟如下：
...
`ReactWebView` 是封裝 Android 原生視圖的元件，當使用我們的自訂元件時，React Native 將渲染此視圖。
...
該類別定義了三個 Android 構造函數，但將其實際實現委託給私有的 `configureComponent` 函數。此函數負責初始化所有元件特定的屬性：在此範例中，您設置了 `WebView` 的佈局，並定義了用於自訂 `WebView` 行為的 `WebClient`。在這段代碼中，`ReactWebView` 在頁面加載完成時通過實作 `WebClient` 的 `onPageFinished` 方法來發送事件。

接著，代碼定義了一個輔助函數來實際發送事件。要發送事件，您需要：
...
當 React 從應用程式接收到渲染特定元件的指令時，React 使用已註冊的視圖管理器來創建視圖並傳遞所有必需的屬性。
...
接著，它覆寫了 `getName` 函數，該函數必須返回與 spec 中 `codegenNativeComponent` 函數調用相同的名稱。
...
最後，您需要在應用程式中註冊 `ReactWebViewPackage`。我們透過修改 `MainApplication` 檔案，將 `ReactWebViewPackage` 加入 `getPackages` 函數返回的套件清單中來實現這一點。
