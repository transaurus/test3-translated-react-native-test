...
1. **使用 Flow 或 TypeScript 等 JavaScript 類型註解語言定義類型化 JavaScript 規格**；
...
React Native 提供了一個名為 [Codegen](/the-new-architecture/what-is-codegen.md) 的工具，該工具可以接受以 TypeScript 或 Flow 撰寫的規格，並為 Android 和 iOS 生成平台特定的程式碼。規格宣告了將在您的原生程式碼和 React Native JavaScript 執行環境之間傳遞的方法和資料類型。Turbo 原生模組既是您的規格，也是您撰寫的原生程式碼，以及從您的規格生成的 Codegen 介面。
