...
- `space-evenly` 在主軸上完全均勻分布子元素。每對相鄰子元素之間、主軸起始端與首個子元素之間、主軸末端與最後一個子元素之間的間距完全相同。
...
:::info
要使 `stretch` 生效，子元素在次要軸上不能有固定尺寸。在以下範例中，設定 `alignItems: stretch` 不會有任何效果，直到移除子元素的 `width: 50`。
:::
...
[`alignSelf`](layout-props#alignself) 的選項和效果與 `alignItems` 相同，但它作用於單個子元素，而非容器內的所有子元素。`alignSelf` 會覆蓋父容器通過 `alignItems` 設定的任何選項。
...
- `stretch`（在網頁版 Yoga 中的預設值）拉伸換行後的內容以匹配容器交叉軸的高度。

- `center` 將換行後的內容在交叉軸上居中對齊。

- `space-between` 在交叉軸上均勻分布換行後的內容，剩餘空間分配在行與行之間。

- `space-around` 在交叉軸上均勻分布換行後的內容，剩餘空間分配在行周圍。容器兩端的空間大小為行間空間的一半。

- `space-evenly` 在交叉軸上完全均勻分布換行後的內容，所有空間大小完全相同。
...
[`flexWrap`](layout-props#flexwrap) 屬性設置在容器上，用於控制當子元素沿主軸溢出容器尺寸時的處理方式。預設情況下，子元素會被強制排列在單一行上（這可能會壓縮元素）。如果允許換行，則會根據需要將項目沿主軸換行成多行。
...
- [`flexBasis`](layout-props#flexbasis) 是一種與軸向無關的方式，用於設定項目沿主軸的預設大小。設定子元素的 `flexBasis` 類似於在父容器為 `flexDirection: row` 時設定該子元素的 `width`，或在父容器為 `flexDirection: column` 時設定其 `height`。項目的 `flexBasis` 是該項目的預設大小，即在進行任何 `flexGrow` 和 `flexShrink` 計算之前的尺寸。
...
- [`flexShrink`](layout-props#flexshrink) 描述當子元素的總大小在主軸上溢出容器尺寸時，應如何沿主軸縮小子元素。`flexShrink` 與 `flexGrow` 非常相似，可以將任何溢出尺寸視為負的剩餘空間來理解。這兩個屬性也能很好地協同工作，允許子元素根據需要進行擴展和收縮。
