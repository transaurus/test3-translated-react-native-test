...
默認情況下，排隊的任務會在一個 `setImmediate` 批次中一起執行。如果使用正數調用 `setDeadline`，則任務只會在截止時間（以 js 事件循環運行時間為準）接近時執行，此時會通過 setTimeout 讓出執行權，允許觸控等事件開始互動並阻止排隊任務執行，使應用更具響應性。
...
正數會使用 setTimeout 在事件循環運行時間達到截止值後安排任何任務，否則所有任務會在一個 setImmediate 批次中執行（默認）。