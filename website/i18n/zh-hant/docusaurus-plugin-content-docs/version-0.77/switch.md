...
iOS 專用：設定開關背景自訂顏色。當開關值為 `false` 或處於禁用狀態（且開關呈半透明）時可見此背景色。
...
_iOS 特別說明_：當開關值為 `false` 時，軌道會縮至邊框內。若要調整縮小後露出的背景色，請使用 [`ios_backgroundColor`](switch.md#ios_backgroundColor)。
...
開關的當前值。設為 `true` 時表示開啟狀態，預設值為 `false`。
