...
AppState 常被用於處理推送通知時，判斷意圖並決定適當的行為。
...
- `background` - 應用程式正在背景執行。使用者可能處於：
  - 其他應用程式中
  - 主畫面
  - [Android] 其他 `Activity` 中（即使是由您的應用程式啟動）
...
此範例永遠只會顯示「當前狀態為：active」，因為應用程式只有在 `active` 狀態時對使用者可見，而 null 狀態只會短暫出現。如果想實驗程式碼，建議使用自己的設備而非嵌入式預覽。
...
設定當 AppState 上指定事件類型發生時要呼叫的函式。有效 `eventType` 值請參閱[上方列表](#events)。回傳 `EventSubscription`。
