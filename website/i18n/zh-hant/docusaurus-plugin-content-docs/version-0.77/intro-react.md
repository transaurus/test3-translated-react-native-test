...
可將組件視為藍圖。函數組件返回的內容會渲染為 **React 元素**，這些元素用於描述屏幕上要顯示的內容。
...
React 和 React Native 使用 **JSX**，這種語法允許在 JavaScript 中直接編寫元素：`<Text>Hello, I am your cat!</Text>`。React 文檔提供 [完整的 JSX 指南](https://react.dev/learn/writing-markup-with-jsx) 可供深入學習。由於 JSX 本質是 JavaScript，可以在其中使用變量。這裡聲明了貓的名字 `name`，並用花括號將其嵌入 `<Text>`。
...
任何 JavaScript 表達式都能在花括號中運行，包括函數調用如 `{getFullName("Rum", "Tum", "Tugger")}`：
...
你已認識 [React Native 的核心組件](intro-react-native-components)。React 允許將這些組件相互嵌套以創建新組件。這種可嵌套、可複用的組件是 React 範式的核心。
...
你可以在咖啡館中放入任意數量的貓咪。每個 `<Cat>` 都會渲染一個獨特的元素——你可以透過 props 自訂這些元素。
...
大多數 React Native 的核心元件也可以透過 props 自訂。例如，使用 [`Image`](image) 時，你可以傳遞名為 [`source`](image#source) 的 prop 來定義它顯示的圖片：
...
`Image` 有[許多不同的 props](image#props)，包括 [`style`](image#style)，它接受一個設計與佈局相關的屬性-值對的 JS 物件。

> 注意 `style` 的寬度和高度周圍的雙大括號 `{{ }}`。在 JSX 中，JavaScript 值使用 `{}` 引用。這在傳遞非字串的 props（如陣列或數字）時很方便：`<Cat food={["魚", "飼料"]} age={2} />`。然而，JS 物件**也**用大括號表示：`{width: 200, height: 200}`。因此，要在 JSX 中傳遞 JS 物件，你必須**再包一層**大括號：`{{width: 200, height: 200}}`

你可以透過 props 和核心元件 [`Text`](text)、[`Image`](image) 和 [`View`](view) 建構許多東西！但要建構互動式內容，你需要狀態。

## State

你可以將 props 視為配置元件渲染方式的參數，而**state** 則像是元件的個人資料儲存空間。state 適用於處理隨時間變化的資料或來自使用者互動的資料。state 賦予你的元件記憶！

> 一般來說，使用 props 來配置元件渲染時的行為。使用 state 來追蹤任何你預期會隨時間變化的元件資料。

以下範例發生在一家貓咪咖啡館，兩隻飢餓的貓咪等待餵食。牠們的飢餓狀態（我們預期會隨時間變化，與牠們的名字不同）被儲存為 state。要餵食貓咪，請按下牠們的按鈕——這將更新牠們的狀態。

你可以透過呼叫 [React 的 `useState` Hook](https://react.dev/learn/state-a-components-memory) 為元件添加 state。Hook 是一種函數，讓你能夠「掛入」React 的功能。例如，`useState` 是一個 Hook，讓你能夠為函數元件添加 state。你可以在 [React 文件](https://react.dev/reference/react) 中了解更多其他類型的 Hook。
...
然後，在元件函數內部呼叫 `useState` 來宣告元件的 state。在此範例中，`useState` 創建了一個 `isHungry` 狀態變數：
...
> 你可以使用 `useState` 追蹤任何類型的資料：字串、數字、布林值、陣列、物件。例如，你可以用 `const [timesPetted, setTimesPetted] = useState(0)` 追蹤貓咪被撫摸的次數！

呼叫 `useState` 會做兩件事：

- 創建一個具有初始值的「狀態變數」——在此範例中，狀態變數是 `isHungry`，其初始值為 `true`
- 創建一個設定該狀態變數值的函數——`setIsHungry`
...
> 看到上面的 `<>` 和 `</>` 了嗎？這些 JSX 片段稱為 [fragments](https://react.dev/reference/react/Fragment)。相鄰的 JSX 元素必須包裹在一個閉合標籤內。Fragments 讓你能做到這點，而無需嵌套多餘的包裹元素如 `View`。

---

現在你已學習了 React 和 React Native 的核心元件，讓我們更深入探討部分核心元件，從 [處理 `<TextInput>`](handling-text-input) 開始。