...
安裝完成後，請在 `~/.zshrc` (或 `~/.bash_profile`) 中新增或更新 `JAVA_HOME` 環境變數。
...
Zulu OpenJDK 發行版提供 **Intel 和 M1 晶片 Mac 專用** 的 JDK。這能確保在 M1 Mac 上獲得比 Intel 版 JDK 更快的建置速度。

若系統已安裝 JDK，建議使用 JDK 17 版本。更高版本可能會導致相容性問題。

<h3>Android 開發環境</h3>

對於 Android 開發新手來說，環境設置可能較為繁瑣。即使已有經驗，仍需進行特定配置。無論如何，請仔細遵循後續步驟。
...
Android Studio 預設會安裝最新版 Android SDK。但建置含原生代碼的 React Native 應用需特定使用 `Android 15 (VanillaIceCream)` SDK。可透過 Android Studio 的 SDK 管理器安裝其他版本。
...
點擊「Next」後選擇「Finish」建立 AVD。此時您應可點擊 AVD 旁的綠色三角形按鈕來啟動它。
