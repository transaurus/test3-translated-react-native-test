...
1. **僅限實體裝置**：開啟設定應用程式，導航至 Safari > 進階，並確保「網頁檢查器」已開啟。
...
遠端 JavaScript 除錯將外部網頁瀏覽器（Chrome）連接到您的應用程式，並在網頁中執行您的 JavaScript 程式碼。這讓您可以使用 Chrome 的除錯工具，就像對待任何網頁應用程式一樣。請注意，瀏覽器環境可能非常不同，且並非所有 React Native 模組在此除錯方式下都能正常運作。
