...
- 各種 `shadow` 屬性（`shadowColor`、`shadowOffset`、`shadowOpacity`、`shadowRadius`）：這些直接映射到平台級 API 暴露的原生對應功能。

`dropShadow` 與 `boxShadow` 的區別如下：

- `dropShadow` 是 `filter` 的一部分，而 `boxShadow` 是獨立的樣式屬性。
- `dropShadow` 是 alpha 遮罩，因此只有具有正 alpha 值的像素才會「投射」陰影。`boxShadow` 則會圍繞元素的邊框盒投射陰影，無論其內容為何（除非是內陰影）。
...
`boxShadow` 和 `dropShadow` 通常比 `shadow` 屬性功能更強大。然而，`shadow` 屬性直接映射到原生平台級 API，因此如果您只需要簡單的陰影效果，建議使用這些屬性。請注意，只有 `shadowColor` 在 Android 和 iOS 上都有效，其他所有 `shadow` 屬性僅在 iOS 上有效。
...
此屬性僅在 Android API 28 及以上版本有效。對於較低 Android API 的類似功能，請使用 [`elevation` 屬性](view-style-props#elevation-android)。
