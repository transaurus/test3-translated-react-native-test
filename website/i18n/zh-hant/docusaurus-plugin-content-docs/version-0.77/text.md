...
在以下範例中，嵌套的標題和正文文字將繼承 `styles.baseText` 的 `fontFamily`，但標題提供了自己額外的樣式。由於實際的換行符，標題和正文將堆疊在彼此之上：
...
Android 和 iOS 都允許您通過為字串的特定範圍添加格式（如粗體或彩色文字）來顯示格式化文字（iOS 上的 `NSAttributedString`，Android 上的 `SpannableString`）。實際上，這非常繁瑣。對於 React Native，我們決定使用網頁的範例來實現相同的效果，即通過嵌套文字來實現。
...
在底層，React Native 會將其轉換為包含以下資訊的扁平 `NSAttributedString` 或 `SpannableString`：
...
您還失去了為整個子樹設定預設字體的能力。同時，`fontFamily` 只接受單一字體名稱，這與 CSS 中的 `font-family` 不同。在應用程式中使用一致的字體和大小的推薦方法是創建一個包含這些樣式的元件 `MyAppText`，並在整個應用程式中使用此元件。您還可以使用此元件來創建更具體的元件，如 `MyAppHeaderText`，用於其他類型的文字。
...
- (實現者) React Native 的實現也得到了簡化。我們不需要在每個元素上都有一個 `fontFamily` 字段，也不需要每次顯示文字節點時都潛在地遍歷樹到根節點。樣式繼承僅編碼在本機 Text 元件內部，不會洩漏到其他元件或系統本身。
...
輔助功能提示（accessibility hint）可幫助使用者在執行操作時，若從輔助功能標籤無法明確理解結果時，提供進一步的說明。
...
覆寫螢幕閱讀器在使用者與元素互動時朗讀的文字。預設情況下，標籤會遍歷所有子元素並以空格分隔累積所有 `Text` 節點的文字內容。
...
在 iOS 上，這些角色會映射到對應的輔助功能特徵（Accessibility Traits）。圖片按鈕（image button）的功能等同於同時設定 'image' 和 'button' 特徵。詳見 [輔助功能指南](accessibility.md#accessibilitytraits-ios)。
...
輔助功能操作允許輔助技術以程式化方式觸發元件的操作。`accessibilityActions` 屬性應包含一組操作物件，每個物件需定義 name 和 label 欄位。
...
當使用者觸發輔助功能操作時呼叫。此函數的唯一參數是一個包含待執行操作名稱的事件物件。
...
在 Android API 23+ 上設定自動斷字頻率，用於決定單詞換行時機。
...
表示可勾選元素的狀態。此屬性可接受布林值或「mixed」字串來表示混合勾選狀態。
...
用於在計算文字佈局（包括換行）後，以省略號截斷文字，使總行數不超過此數值。將此屬性設為 `0` 會取消設定此值，表示不應用任何行數限制。
...
回應者權限已從 `View` 收回。可能是在呼叫 `onResponderTerminationRequest` 後被其他視圖收回，也可能被作業系統未經詢問直接收回（例如 iOS 上的控制中心/通知中心）。
...
當其他 `View` 試圖成為觸控響應者並要求此 `View` 釋放其響應權時觸發。返回 `true` 表示允許釋放。
...
當滾動視圖被禁用時，此屬性定義觸碰點可偏離按鈕多遠才會停用按鈕。停用後若移回範圍內，按鈕會重新啟用。建議傳入常數以減少記憶體分配。
...
設為 `true` 時，文字被按下不會有視覺變化。預設情況下會顯示灰色橢圓高亮。
