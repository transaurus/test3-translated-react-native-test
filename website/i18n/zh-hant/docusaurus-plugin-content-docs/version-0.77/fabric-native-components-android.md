...
`ReactWebView` 是封裝 Android 原生視圖的組件，當使用我們的自定義組件時，React Native 將渲染此視圖。
...
該類別定義了三個 Android 構造函數，但將其實際實現委託給私有的 `configureComponent` 函數。此函數負責初始化所有組件特定的屬性：在此情況下，您設置了 `WebView` 的佈局，並定義了用於自定義 `WebView` 行為的 `WebClient`。在此代碼中，`ReactWebView` 在頁面完成加載時通過實現 `WebClient` 的 `onPageFinished` 方法發出事件。
...
接著，ViewManager 需要定義所有 React 組件屬性將如何更新原生視圖。在此範例中，您需要決定如何處理 React 將在 `WebView` 上設置的 `sourceURL` 屬性。
...
- `getModule` 方法會根據 React Native 需渲染的視圖返回對應的 ViewManager。
