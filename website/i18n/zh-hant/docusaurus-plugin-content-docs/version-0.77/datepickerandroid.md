...
回傳一個 Promise，當使用者選擇日期時會解析為包含 `action`、`year`、`month`（0-11）、`day` 的物件。若使用者關閉對話框，Promise 仍會解析但 `action` 會是 `DatePickerAndroid.dismissedAction`，其他鍵值則為 undefined。**務必**檢查 `action` 是否等於 `DatePickerAndroid.dateSetAction` 後再讀取值。
