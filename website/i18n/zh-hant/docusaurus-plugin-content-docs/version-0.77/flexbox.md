...
在以下範例中，紅色、橙色和綠色視圖都是設有 `flex: 1` 的容器視圖的子元素。紅色視圖使用 `flex: 1`，橙色視圖使用 `flex: 2`，綠色視圖使用 `flex: 3`。**1+2+3 = 6**，這表示紅色視圖將獲得 `1/6` 的空間，橙色視圖獲得 `2/6`，綠色視圖獲得 `3/6`。
...
[`flexDirection`](layout-props#flexdirection) 控制節點子元素的排列方向，這也被稱為主軸。交叉軸則是與主軸垂直的軸，或是換行排列的軸向。
...
佈局 [`direction`](layout-props#direction) 指定了子元素和文字在層次結構中的排列方向。佈局方向也會影響 `start` 和 `end` 所指的邊緣。預設情況下，React Native 使用 LTR（從左到右）佈局方向。在此模式下，`start` 指的是左側，`end` 指的是右側。
...
- `flex-start`（**預設值**）將容器的子元素對齊於主軸的起始處。

- `flex-end` 將容器的子元素對齊於主軸的末端。

- `center` 將容器的子元素對齊於主軸的中心。

- `space-between` 在主軸上均勻分布子元素，剩餘空間分配於子元素之間。

- `space-around` 在主軸上均勻分布子元素，剩餘空間環繞子元素分配。與 `space-between` 相比，使用 `space-around` 會在第一個子元素的起始處和最後一個子元素的末端處也分配空間。

- `space-evenly` 在主軸上完全均勻分布子元素。每對相鄰子元素之間、主軸起始邊緣與第一個子元素之間、主軸末端邊緣與最後一個子元素之間的間距完全相同。
...
- `flex-start` 將容器的子元素對齊於交叉軸的起始處。

- `flex-end` 將容器的子元素對齊於交叉軸的末端。

- `center` 將容器的子元素對齊於交叉軸的中心。

- `baseline` 將容器的子元素沿著共同的基準線對齊。可以將個別子元素設置為其父元素的基準參考。
...
[`alignSelf`](layout-props#alignself) 的選項和效果與 `alignItems` 相同，但它作用於單個子元素，以改變其在父容器中的對齊方式。`alignSelf` 會覆蓋父容器通過 `alignItems` 設置的任何選項。
...
- `flex-start`（**預設值**）將換行的行對齊於容器交叉軸的起始處。

- `flex-end` 將換行的行對齊於容器交叉軸的末端。

- `stretch`（_在網頁上使用 Yoga 時的預設值_）拉伸換行的行以匹配容器交叉軸的高度。

- `center` 將換行的行對齊於容器交叉軸的中心。

- `space-between` 在交叉軸上均勻分布換行的行，剩餘空間分配於行之間。

- `space-around` 在交叉軸上均勻分布換行的行，剩餘空間環繞行分配。容器的兩端分配的空間是行之間空間的一半。

- `space-evenly` 在交叉軸上完全均勻分布換行的行，剩餘空間環繞行分配。每個空間的大小相同。
...
[`flexWrap`](layout-props#flexwrap) 屬性設置在容器上，用於控制當子元素沿主軸溢出容器尺寸時的行為。預設情況下，子元素會被強制排列在單一行內（這可能導致元素縮小）。如果允許換行，則會根據需要沿主軸將項目換行至多行。

當啟用換行時，可使用 `alignContent` 指定行在容器內的排列方式。了解更多[此處](https://www.yogalayout.dev/docs/styling/flex-wrap)。
...
- [`flexBasis`](layout-props#flexbasis) 是一種與軸向無關的方式，用於設定項目沿主軸的預設尺寸。設置子元素的 `flexBasis` 類似於在父容器為 `flexDirection: row` 時設定該子元素的 `width`，或在父容器為 `flexDirection: column` 時設定其 `height`。項目的 `flexBasis` 是該項目的預設尺寸，即在任何 `flexGrow` 和 `flexShrink` 計算之前的尺寸。

- [`flexGrow`](layout-props#flexgrow) 描述容器內剩餘空間應如何沿主軸分配給其子元素。在佈局子元素後，容器會根據子元素指定的 flex grow 值分配剩餘空間。

  `flexGrow` 接受任何 >= 0 的浮點數，預設值為 0。容器會根據子元素的 `flexGrow` 值加權分配剩餘空間。

- [`flexShrink`](layout-props#flexshrink) 描述當子元素的總尺寸沿主軸溢出容器時，應如何縮小子元素。`flexShrink` 與 `flexGrow` 非常相似，可將溢出的尺寸視為負剩餘空間來理解。這兩個屬性也能很好地協同工作，讓子元素根據需要伸縮。

  `flexShrink` 接受任何 >= 0 的浮點數，預設值為 0（在網頁上預設為 1）。容器會根據子元素的 `flexShrink` 值加權縮小它們。

了解更多[此處](https://www.yogalayout.dev/docs/styling/flex-basis-grow-shrink)。
...
- `auto`（**預設值**）React Native 會根據元素內容（無論是其他子元素、文字或圖片）計算其寬度/高度。
...
- `relative`（**預設值**）預設情況下，元素採用相對定位。這表示元素會依照正常的版面流動進行定位，然後根據 `top`、`right`、`bottom` 和 `left` 的值進行相對偏移。此偏移不會影響任何兄弟元素或父元素的位置。

- `absolute` 當採用絕對定位時，元素不會參與正常的版面流動。它會獨立於其兄弟元素進行布局。其位置由 `top`、`right`、`bottom` 和 `left` 的值決定，這些值會將元素相對於其包含塊（containing block）進行定位。

- `static` 當採用靜態定位時，元素會依照正常的版面流動進行定位，並忽略 `top`、`right`、`bottom` 和 `left` 的值。此 `position` 設定還會導致元素不會為其絕對定位的子元素形成包含塊，除非存在其他優先的樣式屬性（例如 `transform`）。這允許絕對定位元素可以定位到非其父元素的元素上。注意：**`static` 僅在新架構（New Architecture）中可用**。
...
元素的包含塊是一個祖先元素，它控制著該元素的位置和大小。
React Native 中包含塊的運作方式與 [網頁上的運作方式](https://developer.mozilla.org/en-US/docs/Web/CSS/Containing_block) 非常相似，但由於缺少某些網頁功能，因此進行了一些簡化。

絕對定位元素的 `top`、`right`、`bottom` 和 `left` 值將相對於其包含塊進行計算。

套用於絕對定位元素的百分比長度（例如：`width: '50%'` 或 `padding: '10%'`）將相對於其包含塊的大小進行計算。例如，如果包含塊的寬度為 100 點，則絕對定位元素上的 `width: 50%` 會使其寬度為 50 點。

以下列表將幫助你確定任何給定元素的包含塊：

- 如果該元素的 `position` 類型為 `relative` 或 `static`，則其包含塊為其父元素。
- 如果該元素的 `position` 類型為 `absolute`，則其包含塊為滿足以下條件之一的最近祖先元素：
  - 其 `position` 類型不為 `static`
  - 其具有 `transform` 屬性

## 深入探討

你可以查看互動式的 [yoga playground](https://www.yogalayout.dev/playground)，以更深入理解 flexbox 的運作方式。

我們已經涵蓋了基礎知識，但你可能還需要許多其他樣式來進行版面配置。完整控制版面的屬性列表記錄在 [這裡](./layout-props.md)。

此外，你可以參考 [Wix Engineers](https://medium.com/wix-engineering/the-full-react-native-layout-cheat-sheet-a4147802405c) 提供的一些範例。