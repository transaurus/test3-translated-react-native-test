...
- [`quad`](easing.md#quad) 二次函數
- [`cubic`](easing.md#cubic) 三次函數

[`poly`](easing.md#poly) 函數可用於實現四次、五次及其他更高次方的函數。
...
預設彈性係數為1，會稍微過衝一次。彈性係數為0則完全不會過衝，而彈性係數N > 1時會過衝約N次。
...
與`Animated.parallel()`一起使用，可創建一個基本效果，即動畫開始時物體會稍微向後移動。
