...
React 元件，封裝了平台專用的 `DrawerLayout`（僅限 Android）。抽屜（通常用於導航）透過 `renderNavigationView` 渲染，而直接子元件則是主視圖（放置內容的地方）。導航視圖最初不會顯示在螢幕上，但可以從 `drawerPosition` 屬性指定的視窗側邊拉出，其寬度可透過 `drawerWidth` 屬性設定。
...
- 鎖定開啟，表示抽屜會保持開啟且不回應手勢。抽屜仍可透過程式碼開啟或關閉（`openDrawer`/`closeDrawer`）。
