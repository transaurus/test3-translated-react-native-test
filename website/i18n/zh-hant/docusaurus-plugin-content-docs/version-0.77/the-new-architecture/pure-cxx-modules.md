...
使用 C++ 編寫模組是實現 Android 與 iOS 平台無關程式碼共享的最佳方式。透過純 C++ 模組，您只需編寫一次邏輯，即可在所有平台直接複用，無需撰寫平台特定程式碼。

本指南將逐步說明如何建立純 C++ Turbo 原生模組：

1. 建立 JS 規格定義檔
2. 配置 Codegen 生成程式碼骨架
3. 實作原生邏輯
...
純 C++ Turbo 原生模組屬於 Turbo 原生模組，需透過規格定義檔（spec file）讓 Codegen 為我們生成程式碼骨架。該規格檔同時也是 JS 層存取 Turbo 原生模組的介面。

規格檔需使用具型別的 JS 方言撰寫，React Native 目前支援 Flow 或 TypeScript。

1. 在應用程式根目錄下建立名為 `specs` 的資料夾
2. 建立新檔案 `NativeSampleModule.ts` 並填入以下程式碼：

:::warning
所有原生 Turbo 模組規格檔必須以 `Native` 前綴命名，否則 Codegen 將忽略這些檔案。
:::
...
此配置指示 Codegen 在 `specs` 資料夾中尋找規格檔，並設定僅為 `modules` 生成程式碼，同時將生成的程式碼命名空間設為 `AppSpecs`。

## 3. 撰寫原生程式碼

撰寫 C++ Turbo 原生模組可實現 Android 與 iOS 的程式碼共享。因此我們只需編寫一次程式碼，後續將說明如何調整各平台配置以載入這些 C++ 程式碼。

1. 在與 `android` 和 `ios` 同層級目錄建立名為 `shared` 的資料夾
...
- `NativeSampleModule.h` 檔案是純 C++ TurboModule 的標頭檔。`include` 語句確保我們包含了由 Codegen 生成的規範，這些規範包含了我們需要實作的介面和基礎類別。
- 該模組位於 `facebook::react` 命名空間中，以便存取該命名空間中的所有類型。
- `NativeSampleModule` 類別是實際的 Turbo Native Module 類別，它繼承了 `NativeSampleModuleCxxSpec` 類別，後者包含了一些黏合程式碼和樣板程式碼，使這個類別能夠作為 Turbo Native Module 運作。
...
- 使用 `target_sources` 指令添加我們需要構建的模組 C++ 原始碼。預設情況下，React Native 已經會將預設原始碼填入 `appmodules` 庫，這裡我們包含了我們自定義的部分。你可以看到我們需要從 `jni` 資料夾回溯到 `shared` 資料夾，那裡是我們的 C++ Turbo Module 所在的位置。
...
最後一步是在運行時註冊新的 C++ Turbo Native Module，以便當 JS 請求 C++ Turbo Native Module 時，應用程式知道去哪裡找到它並返回它。
...
我們需要執行的第一步是每次準備 iOS 應用程式時的常規步驟。CocoaPods 是我們用來設定和安裝 React Native 相依性的工具，作為流程的一部分，它也會為我們執行 Codegen。
...
如果你的應用程式屬於此類別，請跳過將 AppDelegate 遷移到 Swift 的步驟，並繼續為你的應用程式的 AppDelegate 使用 Objective-C++。

React Native 核心主要使用 C++ 開發，以鼓勵 iOS、Android 和其他平台間的程式碼共享。Swift 和 C++ 之間的互通性尚未成熟或穩定。我們正在尋找方法填補這一缺口，讓你也遷移到 Swift。
:::
...
1. 匯入 `RCTAppDelegate+Protected` 標頭檔，讓 AppDelegate 可見它符合 `RCTTurboModuleManagerDelegate` 協議。
2. 匯入純 C++ Native Turbo Module 介面 `NativeSampleModule.h`
3. 覆寫 C++ 模組的 `getTurboModule` 方法，讓當 JS 端要求名為 `NativeSampleModule` 的模組時，應用程式知道要回傳哪個模組。

如果你現在從 Xcode 建置你的應用程式，應該能成功建置。

## 5. 測試你的程式碼

現在是時候從 JS 存取我們的 C++ Turbo Native Module 了。為此，我們需要修改 `App.tsx` 檔案，以導入 Turbo Native Module 並在程式碼中呼叫它。
...
- `import SampleTurboModule from './specs/NativeSampleModule';`：這一行在應用程式中導入 Turbo Native Module，
- `const revString = SampleTurboModule.reverseString(value);` 在 `onPress` 回呼中：這是你在應用程式中使用 Turbo Native Module 的方式。

:::warning
為了這個範例並保持簡潔，我們直接在應用程式中導入規格檔案。
