...
要啟用推播通知，請先[透過 Apple 設定您的通知服務](https://developer.apple.com/documentation/usernotifications/setting_up_a_remote_notification_server)與後端系統。
...
實作 `userNotificationCenter:didReceiveNotificationResponse:withCompletionHandler:` 以處理使用者點擊通知的情況，通常用於使用者點擊背景通知開啟應用程式時。但若您在前景通知處理方法中設定要顯示通知，則此方法也會在使用者點擊前景通知時被呼叫。此時，您應僅在其中一個回呼中通知 `RCTPushNotificationManager`。
...
- `category` : 此通知的分類，用於可操作通知（選填）。例如包含回覆或按讚等額外操作的通知。
...
- `fireDate` : 通知觸發的具體時間。排程通知可使用 `fireDate` 或 `fireIntervalSeconds` 設定，其中 `fireDate` 優先級更高。
- `fireIntervalSeconds` : 從當前時間起算，多少秒後顯示通知。
- `userInfo` : 包含額外通知資料的物件（可選）。
- `category` : 此通知的分類，用於具備額外操作的通知（可選）。例如包含「回覆」或「按讚」等動作的通知。
...
向 iOS 請求通知權限，並以對話框提示用戶。預設情況下，此方法會請求所有通知權限，但您可以選擇性地指定要請求的權限。支援的權限包括：
...
您應該僅在極少數情況下調用此方法，例如當應用程式的新版本移除了對所有類型遠端通知的支援時。用戶可以透過設定應用程式暫時阻止應用程式接收遠端通知。透過此方法取消註冊的應用程式隨時可以重新註冊。
...
若因某些原因仍依賴 `application:didReceiveRemoteNotification:fetchCompletionHandler:`，則需在 iOS 端設定事件處理：
