...
回傳一個布林值，判斷裝置是否為 Apple Vision。_若您使用[Apple Vision Pro (專為 iPad 設計)](https://developer.apple.com/documentation/visionos/checking-whether-your-app-is-compatible-with-visionos)，`isVision` 會是 `false` 但 `isPad` 會是 `true`_
...
回傳一個布林值，判斷應用程式是否在開發者模式中執行且測試標記已設定。
...
這會讓容器在所有平台上擁有 `flex: 1` 的樣式，在 Android 上背景顏色為綠色，在 iOS 上為紅色，其他平台則為藍色。

由於對應平台鍵值的型別可以是 `any`，[`select`](platform.md#select) 方法也可用於回傳平台特定的元件，如下所示：
