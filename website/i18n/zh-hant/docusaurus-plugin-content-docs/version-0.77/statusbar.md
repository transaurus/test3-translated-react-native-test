...
控制應用程式狀態列的元件。狀態列通常是螢幕頂端的區域，顯示當前時間、Wi-Fi和行動網路資訊、電池電量或其他狀態圖示。

### 與導航器搭配使用

可以同時掛載多個`StatusBar`元件。屬性會按照`StatusBar`元件掛載的順序合併。
...
:::warning
由於Android 15引入了邊到邊強制執行，在API級別35中設定狀態列背景顏色已被棄用，設定後不會產生效果。更多關於[邊到邊建議的資訊可在此處閱讀](https://github.com/react-native-community/discussions-and-proposals/discussions/827)。
:::
...
狀態列是否為半透明。當設為`true`時，應用程式會繪製在狀態列下方。這在使用半透明狀態列顏色時很有用。

:::warning
由於Android 15引入了邊到邊強制執行，在API級別35中將狀態列設為半透明已被棄用，設定後不會產生效果。更多關於[邊到邊建議的資訊可在此處閱讀](https://github.com/react-native-community/discussions-and-proposals/discussions/827)。
:::
