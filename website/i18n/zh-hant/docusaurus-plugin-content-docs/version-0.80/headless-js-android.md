...
Headless JS 是一種在應用程式處於後台時執行 JavaScript 任務的方式。例如，可用於同步最新數據、處理推送通知或播放音樂。
...
默認情況下，Headless JS 任務不會進行任何重試。要實現重試，你需要創建一個 `HeadlessJsRetryPolicy` 並拋出特定的 `Error`。
...
- 默認情況下，如果嘗試在前台運行任務，應用程式會崩潰。這是為了防止開發者因在任務中執行大量工作而拖慢 UI。你可以傳遞第四個 `boolean` 參數來控制此行為。
- 如果從 `BroadcastReceiver` 啟動服務，請確保在 `onReceive()` 返回之前調用 `HeadlessJsTaskService.acquireWakeLockNow()`。

## 使用範例

可以從 Java API 啟動服務。首先，你需要決定何時啟動服務並相應地實現你的解決方案。以下是一個響應網絡連接變化的範例。

以下代碼顯示了註冊廣播接收器的 Android 清單文件部分。
...
廣播接收器然後在 `onReceive` 函數中處理廣播的意圖。這是檢查應用程式是否處於前台的好地方。如果應用程式不在前台，我們可以準備啟動意圖，不帶信息或使用 `putExtra` 捆綁額外信息（請注意 bundle 只能處理可打包的值）。最後啟動服務並獲取 wakelock。
