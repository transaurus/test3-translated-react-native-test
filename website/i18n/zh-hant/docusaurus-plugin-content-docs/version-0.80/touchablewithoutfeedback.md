...
`TouchableWithoutFeedback` 僅支援單個子元素。若需多個子組件，請將它們包裹在 View 中。重要的是，`TouchableWithoutFeedback` 通過克隆其子元素並應用響應屬性來工作。因此，任何中介組件都必須將這些屬性傳遞給底層的 React Native 組件。
...
此值指示當顏色反轉開啟時，此視圖是否應被反轉。設為 `true` 將告訴視圖即使顏色反轉開啟也不進行反轉。
...
當為 `true` 時，表示該視圖是一個無障礙元素。預設情況下，所有可觸碰元素都是無障礙的。
...
覆寫當用戶與元素互動時屏幕閱讀器朗讀的文字。預設情況下，標籤是通過遍歷所有子元素並累積所有以空格分隔的 `Text` 節點來構建的。
...
此值指示屏幕閱讀器在用戶與元素互動時應使用的語言。它應遵循 [BCP 47 規範](https://www.rfc-editor.org/info/bcp47)。
...
無障礙提示幫助用戶理解當他們對無障礙元素執行操作時會發生什麼，當該結果從無障礙標籤中不明確時。
...
無障礙操作允許輔助技術以程式化方式觸發元件的操作。`accessibilityActions`屬性應包含操作對象列表，每個操作對象需包含名稱和標籤欄位。
...
> 觸碰區域永遠不會超出父視圖的邊界，並且如果觸碰命中兩個重疊的視圖，兄弟視圖的 Z-index 始終具有優先權。
...
如果在 `onPressIn` 之後的時間超過 370 毫秒，則調用此函數。此時間段可以通過 [`delayLongPress`](#delaylongpress) 自定義。
...
當觸碰被釋放時調用，但如果被取消（例如被搶佔響應者鎖的滾動）則不調用。第一個函數參數是一個 [PressEvent](pressevent) 形式的事件。
...
一旦可觸碰元件被按下即調用，甚至在 onPress 之前調用。這在進行網絡請求時很有用。第一個函數參數是一個 [PressEvent](pressevent) 形式的事件。
...
一旦觸碰被釋放即調用，甚至在 onPress 之前調用。第一個函數參數是一個 [PressEvent](pressevent) 形式的事件。
...
當滾動視圖被禁用時，這定義了你的觸碰可以移出按鈕多遠的距離，然後才會停用按鈕。一旦停用，嘗試將其移回，你會看到按鈕再次被激活！在滾動視圖被禁用時多次來回移動它。確保傳入一個常量以減少內存分配。
