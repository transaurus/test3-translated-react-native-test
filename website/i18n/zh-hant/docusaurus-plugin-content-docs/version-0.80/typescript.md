...
TypeScript 也可與 [Expo][expo] 一起使用，後者維護著 TypeScript 模板，或在專案中添加 `.ts` 或 `.tsx` 文件時會提示您自動安裝和配置 TypeScript。
...
:::note
此命令會添加每個依賴項的最新版本。可能需要更改版本以匹配專案中現有的套件。您可以使用 [React Native Upgrade Helper](https://react-native-community.github.io/upgrade-helper/) 等工具來查看 React Native 提供的版本。
:::

2. 添加 TypeScript 配置文件。在專案根目錄中創建一個 `tsconfig.json`：
...
React Native 預設新專案使用 TypeScript，但仍可使用 JavaScript。副檔名為 `.jsx` 的文件將被視為 JavaScript 而非 TypeScript，且不會進行類型檢查。JavaScript 模組仍可被 TypeScript 模組導入，反之亦然。

## TypeScript 與 React Native 的運作方式

開箱即用，TypeScript 源碼在打包時會由 [Babel][babel] 進行轉換。我們建議僅使用 TypeScript 編譯器進行類型檢查。這是新創建專案中 `tsc` 的預設行為。如果您有現有的 TypeScript 代碼需要移植到 React Native，使用 Babel 而非 TypeScript 時會有[一兩個注意事項][babel-7-caveats]。

## React Native + TypeScript 的樣貌

您可以透過 `React.Component<Props, State>` 為 React 組件的 [Props](props) 和 [State](state) 提供介面，這將在使用 JSX 處理該組件時提供類型檢查和編輯器自動完成功能。
...
1. 編輯您的 `tsconfig.json` 以設置[自定義路徑映射][path-map]。設置 `src` 根目錄下的任何內容無需前綴路徑引用即可使用，並允許通過 `tests/File.tsx` 訪問任何測試文件：
