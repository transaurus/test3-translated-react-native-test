...
`navigationBarTranslucent` 屬性決定模態框是否應延伸至系統導覽列下方。但需同時將 `statusBarTranslucent` 設為 `true` 才能使導覽列透明化。
...
`onDismiss` 屬性允許傳入一個函式，該函式將在模態框關閉後被呼叫。
...
`onOrientationChange` 回調函式會在模態框顯示期間裝置方向改變時被呼叫。提供的方向僅為 'portrait' 或 'landscape'。此回調也會在初始渲染時被呼叫，無論當前方向為何。
...
當用戶在 Android 上點擊硬體返回鍵或在 Apple TV 上點擊選單鍵時，會呼叫 `onRequestClose` 回調函式。由於此為必要屬性，請注意只要模態框處於開啟狀態，`BackHandler` 事件將不會被觸發。
...
`onShow` 屬性允許傳入一個函式，該函式將在模態框顯示後被呼叫。
...
`presentationStyle` 屬性控制模態框的呈現方式（通常在較大裝置如 iPad 或加大版 iPhone 上）。詳見 https://developer.apple.com/reference/uikit/uimodalpresentationstyle。

可能的值：

- `fullScreen` 完全覆蓋整個螢幕
- `pageSheet` 以直向寬度居中顯示（僅在較大裝置上）
