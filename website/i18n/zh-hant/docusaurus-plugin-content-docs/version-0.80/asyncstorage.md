...
建議在輕量使用之外的情境中，應基於 `AsyncStorage` 進行抽象封裝而非直接使用，因其運作於全域層級。

在 iOS 上，`AsyncStorage` 由原生代碼實現，小數值會儲存於序列化字典中，較大數值則存於獨立檔案。Android 平台會根據可用性使用 [RocksDB](https://rocksdb.org/) 或 SQLite。

`AsyncStorage` 的 JavaScript 代碼為門面模式，提供清晰的 JavaScript API、真實的 `Error` 物件及非批量操作函式。每個 API 方法均返回 `Promise` 物件。
...
為 `key` 設定數值並在完成時觸發回調。返回 `Promise` 物件。
...
將輸入值與現有 `key` 值合併（假設兩者均為 JSON 字串）。返回 `Promise` 物件。
...
清除 _所有_ 客戶端、函式庫等的 `AsyncStorage` 資料。通常不應直接呼叫此方法，請改用 `removeItem` 或 `multiRemove` 僅清除應用自身的鍵值。返回 `Promise` 物件。
...
取得應用已知的 _所有_ 鍵值（包含所有呼叫者與函式庫）。返回 `Promise` 物件。
...
此方法用於批量儲存多個鍵值對。操作完成時將返回單一包含錯誤的回調：
...
呼叫此方法可批量刪除 `keys` 陣列中的所有鍵。返回一個 `Promise` 物件。
...
批量合併指定鍵集的現有值與新值（假設值為JSON字串）。返回一個 `Promise` 物件。
