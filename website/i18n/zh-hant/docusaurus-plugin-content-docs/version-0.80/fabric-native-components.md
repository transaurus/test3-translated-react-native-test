...
您的規格必須使用 [TypeScript](https://www.typescriptlang.org/) 或 [Flow](https://flow.org/) 定義（詳見 [Codegen](the-new-architecture/what-is-codegen) 文件）。Codegen 將據此生成 C++、Objective-C++ 和 Java 程式碼，用於連接平台程式碼與 React 運行的 JavaScript 運行環境。

規格文件必須命名為 `<模組名稱>NativeComponent.{ts|js}` 才能與 Codegen 協同工作。後綴 `NativeComponent` 不僅是慣例，更是 Codegen 識別規格文件的關鍵字。
...
完成 Codegen 的基礎配置後，我們需要準備原生程式碼來對接生成的程式碼。

請注意，對於 iOS 平台，我們需宣告式地將規格導出的 JS 元件名稱（`CustomWebView`）與實作該元件原生功能的 iOS 類別進行映射。

## 2. 建置原生程式碼

現在是時候撰寫原生平台程式碼，當 React 需要渲染視圖時，平台能夠建立正確的原生視圖並將其顯示在畫面上。
