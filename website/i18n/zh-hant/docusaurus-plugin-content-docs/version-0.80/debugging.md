...
- 在 CLI 中按下 <kbd>j</kbd>（執行 `npx react-native start` 時）。

首次啟動時，DevTools 會開啟歡迎面板，並同時展開控制台抽屜，您可以在這裡查看日誌並與 JavaScript 執行環境互動。從視窗頂部可導航至其他面板，包括整合的 React 元件檢查器和效能分析工具。

![React Native DevTools 開啟至「Welcome」面板](/docs/assets/debugging-rndt-welcome.jpg)

React Native DevTools 由內建於 React Native 的專用除錯架構驅動，並採用客製化的 [Chrome DevTools](https://developer.chrome.com/docs/devtools) 前端介面。這讓我們能提供與瀏覽器一致且熟悉的除錯功能，同時具備深度整合與端到端的可靠性。
...
React Native DevTools 專為除錯 React 應用問題而設計，並非用來取代原生工具。若您需要檢查 React Native 底層平台相關內容（例如開發原生模組時），請使用 Xcode 和 Android Studio 提供的除錯工具（參見[除錯原生程式碼](/docs/next/debugging-native-code)）。
...
LogBox 是應用程式內的工具，當您的應用記錄警告或錯誤時會顯示通知。
...
在 Android 和 iOS 上，開發期間可透過開發者選單選擇 **「Perf Monitor」** 來切換應用內效能疊加層。詳情請參閱[效能文件](/docs/performance)。
