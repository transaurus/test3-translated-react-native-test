...
select 方法會回傳最適合當前執行平台的數值。也就是說，如果您在手機上執行，會優先採用 `android` 和 `ios` 鍵值。若未指定這些鍵值，則會使用 `native` 鍵值，接著是 `default` 鍵值。

`config` 參數是一個包含以下鍵值的物件：
...
這會讓容器在所有平台上都有 `flex: 1` 的樣式，在 Android 上有綠色背景，在 iOS 上有紅色背景，在其他平台上則有藍色背景。

由於對應平台鍵值的數值可以是 `any` 類型，[`select`](platform.md#select) 方法也可用來回傳平台特定的元件，如下所示：
