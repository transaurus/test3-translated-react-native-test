...
在 iOS 平台上，可自訂背景顏色。當開關值為`false`或開關被禁用時（此時開關呈半透明狀態），可看到此背景色。
...
_iOS_：當開關值為`false`時，軌道會向邊界收縮。若要改變收縮後露出的背景顏色，請使用 [`ios_backgroundColor`](switch.md#ios_backgroundColor)。
