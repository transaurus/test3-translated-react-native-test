...
- 若停用 Hermes，React Native 將改用 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)（Safari 瀏覽器的 JavaScript 引擎）。請注意在 iOS 上，由於應用程式缺乏可寫入的執行記憶體，JavaScriptCore 不會使用 JIT 編譯。
...
React Native 內建 [Babel JavaScript 編譯器](https://babeljs.io)。詳情請查閱 [Babel 文件](https://babeljs.io/docs/plugins/#transform-plugins)中關於其支援的轉換功能。
