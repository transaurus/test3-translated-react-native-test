...
現在需要編寫一些 Android 平台代碼，以確保應用程式關閉後 `localStorage` 仍能保留數據。

第一步是實現生成的 `NativeLocalStorageSpec` 介面：
...
接著我們需要創建 `NativeLocalStoragePackage`。它通過將模組封裝為基礎原生套件，提供了一個在 React Native 運行時註冊模組的對象：
...
:::info
後續您將學習如何將原生模組作為 [npm 套件](the-new-architecture/create-module-library.md#publish-the-library-on-npm) 分發，我們的構建工具會自動為您進行鏈接。
:::
