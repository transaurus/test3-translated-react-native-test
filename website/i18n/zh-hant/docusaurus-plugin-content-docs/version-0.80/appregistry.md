...
當需要「停止」應用程式並銷毀視圖時，請使用傳入 `runApplication` 的標籤呼叫 `AppRegistry.unmountApplicationComponentAtRootTag`。這兩者應始終配對使用。

為確保 JavaScript 執行環境在其他模組載入前已完成設置，應在 `require` 序列的早期階段引入 `AppRegistry`。
