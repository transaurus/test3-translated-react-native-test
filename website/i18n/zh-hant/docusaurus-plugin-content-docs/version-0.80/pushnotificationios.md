...
- `category` : 此通知的類別，用於可操作通知（可選）。例如包含「回覆」或「讚」等額外操作的通知。
...
- `fireIntervalSeconds` : 從當前時間起算，多少秒後顯示通知。
- `userInfo` : 包含額外通知資料的物件（可選）。
...
- `notification`：當收到遠端通知時觸發。處理程序將接收一個 `PushNotificationIOS` 實例。此事件會處理在前景收到的通知，或從背景點擊開啟應用程式的通知。
- `localNotification`：當收到本地通知時觸發。處理程序將接收一個 `PushNotificationIOS` 實例。此事件會處理在前景收到的通知，或從背景點擊開啟應用程式的通知。
- `register`：當用戶成功註冊遠端通知時觸發。處理程序將接收一個代表裝置令牌（deviceToken）的十六進制字串。
- `registrationError`：當用戶註冊遠端通知失敗時觸發。通常因 APNS 問題或裝置為模擬器而發生。處理程序將接收 `{message: string, code: number, details: any}`。
...
此方法返回一個 Promise，當用戶接受或拒絕請求時，或權限先前已被拒絕時，該 Promise 會解析。Promise 解析後的值為請求完成後的權限狀態。
...
您應僅在極少數情況下呼叫此方法，例如當應用程式的新版本移除對所有類型遠端通知的支援時。用戶可以透過「設定」應用暫時阻止應用接收遠端通知。透過此方法取消註冊的應用程式隨時可以重新註冊。
...
此方法返回一個 Promise。如果應用程式是透過點擊推送通知啟動的，則該 Promise 會解析為一個 `PushNotificationIOS` 類型的物件，代表被點擊的通知。否則，解析為 `null`。
...
此方法返回一個 Promise，解析為當前的通知授權狀態。可能的狀態值請參閱 [UNAuthorizationStatus](https://developer.apple.com/documentation/usernotifications/unauthorizationstatus?language=objc)。
