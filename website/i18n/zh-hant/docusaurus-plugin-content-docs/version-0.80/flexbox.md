...
:::caution
Flexbox 在 React Native 中的運作方式與網頁 CSS 基本相同，但存在幾項例外：
預設值不同，`flexDirection` 預設為 `column` 而非 `row`，`alignContent` 預設為 `flex-start` 而非 `stretch`，`flexShrink` 預設為 `0` 而非 `1`，且 `flex` 參數僅支援單一數值。
:::

## Flex 彈性佈局

[`flex`](layout-props#flex) 屬性決定了子元素如何沿主軸**「填滿」**可用空間。空間將根據各元素的 flex 屬性值按比例分配。

在以下範例中，紅色、橘色和綠色視圖都是設有 `flex: 1` 的容器視圖的子元素。紅色視圖使用 `flex: 1`，橘色視圖使用 `flex: 2`，綠色視圖使用 `flex: 3`。**1+2+3 = 6**，這表示紅色視圖將獲得 1/6 的空間，橘色視圖獲得 2/6，綠色視圖則獲得 3/6 的空間。
...
- `column`（**預設值**）子元素從上到下排列。若啟用換行，下一行會從容器頂部第一個項目的右側開始。
- `row` 子元素從左到右排列。若啟用換行，下一行會從容器左側第一個項目的下方開始。
- `column-reverse` 子元素從下到上排列。若啟用換行，下一行會從容器底部第一個項目的右側開始。
- `row-reverse` 子元素從右到左排列。若啟用換行，下一行會從容器右側第一個項目的下方開始。

可參考[此處](https://www.yogalayout.dev/docs/styling/flex-direction)了解更多資訊。
...
佈局[方向](layout-props#direction)決定了子元素和文字在層級結構中的排列方向，同時影響 `start` 和 `end` 所指的邊緣。React Native 預設採用 LTR（左至右）佈局方向，此時 `start` 表示左側，`end` 表示右側。
...
- `space-around` 在主軸上均勻分布子元素，剩餘空間環繞子元素分配。與 `space-between` 相比，`space-around` 會在第一個子元素的起始處和最後一個子元素的末端也分配空間。

- `space-evenly` 在主軸上完全均勻分布子元素。每對相鄰子元素之間的間距、主軸起始邊緣與第一個子元素的間距，以及主軸末端邊緣與最後一個子元素的間距，都完全相同。
...
[alignContent](layout-props#aligncontent) 定義了沿著交叉軸的行的分布方式。此屬性僅在使用 `flexWrap` 將項目換行至多行時有效。

- `flex-start`（**預設值**）將換行後的行對齊至容器交叉軸的起始處。

- `flex-end` 將換行後的行對齊至容器交叉軸的末端。

- `stretch`（在網頁上使用 Yoga 時的預設值）拉伸換行後的行以匹配容器交叉軸的高度。

- `center` 將換行後的行對齊至容器交叉軸的中心。

- `space-between` 在容器交叉軸上均勻分布換行後的行，剩餘空間分配於行之間。

- `space-around` 在容器交叉軸上均勻分布換行後的行，剩餘空間環繞行分配。容器的兩端分配的空間大小為行之間空間的一半。

- `space-evenly` 在容器交叉軸上完全均勻分布換行後的行，剩餘空間環繞行分配。每個空間的大小相同。
...
[`flexWrap`](layout-props#flexwrap) 屬性設置在容器上，用於控制當子元素沿主軸溢出容器尺寸時的處理方式。預設情況下，子元素會被強制排列在同一行（這可能會壓縮元素）。如果允許換行，則會根據需要沿主軸將項目換行至多行。
...
- [`flexBasis`](layout-props#flexbasis) 是一種與軸向無關的方式，用於設定項目沿主軸的預設大小。設置子元素的 `flexBasis` 類似於在父容器為 `flexDirection: row` 時設置該子元素的 `width`，或在父容器為 `flexDirection: column` 時設置其 `height`。項目的 `flexBasis` 是該項目的預設大小，即在進行任何 `flexGrow` 和 `flexShrink` 計算之前的大小。

- [`flexGrow`](layout-props#flexgrow) 描述容器內應如何沿主軸分配剩餘空間給其子元素。在佈局其子元素後，容器會根據子元素指定的 flex grow 值分配剩餘空間。

  `flexGrow` 接受任何大於或等於 0 的浮點數，預設值為 0。容器會根據子元素的 `flexGrow` 值加權分配剩餘空間。

- [`flexShrink`](layout-props#flexshrink) 描述當子元素的總大小在主軸上溢出容器時，應如何沿主軸收縮子元素。`flexShrink` 與 `flexGrow` 非常相似，可以將任何溢出的空間視為負的剩餘空間來理解。這兩個屬性也能很好地協同工作，允許子元素根據需要增長或收縮。

  `flexShrink` 接受任何大於或等於 0 的浮點數，預設值為 0（在網頁上，預設值為 1）。容器會根據子元素的 `flexShrink` 值加權收縮其子元素。
...
- `absolute` 當元素採用絕對定位時，它不會參與正常的佈局流，而是獨立於其兄弟元素進行佈局。其位置由 `top`、`right`、`bottom` 和 `left` 的值決定，這些值會將元素相對於其包含塊（containing block）進行定位。

- `static` 當元素採用靜態定位時，它會依照正常的佈局流進行定位，並忽略 `top`、`right`、`bottom` 和 `left` 的值。此 `position` 設定還會導致元素不會為其絕對定位的子代形成包含塊，除非存在其他優先的樣式屬性（例如 `transform`）。這允許 `absolute` 元素可以定位到非其父元素的物件上。請注意，**`static` 僅在新架構（New Architecture）中可用**。
...
元素的包含塊是一個祖先元素，它控制該元素的位置和大小。React Native 中包含塊的運作方式與 [網頁上的運作方式](https://developer.mozilla.org/en-US/docs/Web/CSS/Containing_block) 非常相似，但由於缺少某些網頁功能而有所簡化。

絕對定位元素的 `top`、`right`、`bottom` 和 `left` 值將相對於其包含塊進行計算。

套用於絕對定位元素的百分比長度（例如：`width: '50%'` 或 `padding: '10%'`）將根據其包含塊的大小進行計算。例如，如果包含塊的寬度為 100 點，則絕對定位元素上的 `width: 50%` 會使其寬度為 50 點。

以下列表可幫助您確定任何給定元素的包含塊：

- 如果該元素的 `position` 類型為 `relative` 或 `static`，則其包含塊為其父元素。
- 如果該元素的 `position` 類型為 `absolute`，則其包含塊為滿足以下任一條件的最接近的祖先元素：
...
我們已經涵蓋了基礎知識，但您可能還需要許多其他樣式來進行佈局。控制佈局的完整屬性列表記錄在 [這裡](./layout-props.md)。
