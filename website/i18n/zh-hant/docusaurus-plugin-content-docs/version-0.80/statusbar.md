...
對於不適合使用元件的情況，該元件也提供了以靜態函數形式公開的命令式API。但不建議同時對同一個屬性使用靜態API和元件，因為靜態API設置的任何值都會在下一次渲染時被元件設置的值覆蓋。
...
:::warning
由於Android 15引入的邊到邊(edge-to-edge)強制規範，在API級別35中設置狀態列背景顏色已被棄用，設置後將無效。更多關於[邊到邊建議的資訊可在此查閱](https://github.com/react-native-community/discussions-and-proposals/discussions/827)。
:::
...
:::warning
由於Android 15引入的邊到邊(edge-to-edge)強制規範，在API級別35中設置狀態列為透明已被棄用，設置後將無效。更多關於[邊到邊建議的資訊可在此查閱](https://github.com/react-native-community/discussions-and-proposals/discussions/827)。
:::
