...
在上述堆疊追蹤中，類似 `p@1:132161` 的條目是經過最小化的函數名稱與位元組碼偏移量。要除錯這些呼叫，我們需要將其轉換為檔案、行數與函數名稱，例如 `AwesomeProject/App.js:54:initializeMap`。此過程稱為**符號化**。

您可以透過將堆疊追蹤與生成的原始碼映射傳遞給 [`metro-symbolicate`](http://npmjs.com/package/metro-symbolicate)，來符號化上述的最小化函數名稱與位元組碼。

### 啟用原始碼映射

符號化堆疊追蹤需要原始碼映射。請確認在建置目標平台的設定中已啟用原始碼映射功能。
...
- 建置過程可能會生成多個原始碼映射。請確保使用範例中所示位置的對應檔案。
- 請確認使用的原始碼映射與當機應用程式的提交版本完全匹配。原始碼的微小變更可能導致偏移量大幅差異。
