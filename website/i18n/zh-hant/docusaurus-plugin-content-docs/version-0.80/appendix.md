...
- **Spec** - 用於描述 Turbo Native 模組或 Fabric Native 元件 API 的 TypeScript 或 Flow 程式碼。由 **Codegen** 用來生成樣板程式碼。

- **原生模組 (Native Modules)** - 不包含使用者介面(UI)的原生函式庫。例如持久化儲存、通知、網路事件等。這些可透過函式和物件形式供 JavaScript 應用程式碼存取。
- **原生元件 (Native Component)** - 可透過 React 元件形式供應用程式 JavaScript 碼使用的原生平台視圖。
...
:::info
您可能也會發現參考 React Native 核心模組的 JavaScript 規範很有幫助。這些規範位於 React Native 代碼庫中的 `Libraries/` 目錄內。
:::