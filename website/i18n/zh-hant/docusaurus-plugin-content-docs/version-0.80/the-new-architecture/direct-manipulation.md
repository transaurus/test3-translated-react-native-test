...
另一個常見用例是編輯 TextInput 的值。當 `bufferDelay` 較低且用戶輸入速度極快時，TextInput 的 `controlled` 屬性有時會漏字。部分開發者選擇完全跳過此屬性，改在必要時用 `setNativeProps` 直接操作 TextInput 值。
...
若更新的屬性同時由渲染函數管理，可能導致不可預測的錯誤。因為每當元件重新渲染且該屬性變化時，先前透過 `setNativeProps` 設置的值會被完全忽略並覆寫。