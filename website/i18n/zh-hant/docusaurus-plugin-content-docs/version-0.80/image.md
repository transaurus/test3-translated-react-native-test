...
> 注意：上述列出的版本可能未及時更新。請查閱主代碼庫中的 [`packages/react-native/gradle/libs.versions.toml`](https://github.com/facebook/react-native/blob/main/packages/react-native/gradle/libs.versions.toml) 以查看特定標記版本中使用的 fresco 版本。
...
當圖片調整大小時，由 `capInsets` 指定的尺寸角落將保持固定大小，但圖片的中心內容和邊框會被拉伸。這對於創建可調整大小的圓角按鈕、陰影和其他可調整大小的資源非常有用。更多資訊請參閱 [Apple 官方文件](https://developer.apple.com/library/ios/documentation/UIKit/Reference/UIImage_Class/index.html#//apple_ref/occ/instm/UIImage/resizableImageWithCapInsets)。
...
類似於 `source`，此屬性表示用於渲染圖片載入指示器的資源。載入指示器會一直顯示，直到圖片準備好顯示（通常是圖片下載完成後）。
...
在圖片部分載入完成時觸發。「部分載入」的定義取決於載入器，但通常用於漸進式 JPEG 載入。
...
一個字串，表示在獲取資源時使用哪個 referrer。設定圖片請求中 `Referrer-Policy` 標頭的值，功能類似於 HTML 中的 `referrerpolicy` 屬性。
...
- `scale`: 圖像會被縮小或放大繪製。與 `resize` 相比，`scale` 速度更快（通常由硬體加速）且能產生更高品質的圖像。當圖像小於視圖時應使用此方法。若圖像略大於視圖時也應使用。

- `none`: 不進行任何採樣，圖像會以其完整解析度顯示。此方法僅應在極少數情況下使用，因為當 Android 嘗試渲染消耗過多記憶體的圖像時會拋出運行時異常，這被視為不安全行為。
...
- `cover`: 均勻縮放圖像（保持長寬比），使：
  - 圖像的寬高至少有一個維度與視圖對應維度（減去內距）相等
  - 圖像的兩個維度都會大於或等於視圖對應維度（減去內距）

- `contain`: 均勻縮放圖像（保持長寬比），使圖像的兩個維度都會小於或等於視圖對應維度（減去內距）。

- `stretch`: 獨立縮放寬度和高度，這可能改變原始圖像的長寬比。

- `repeat`: 重複圖像以填滿視圖框架。圖像會保持其尺寸和長寬比，除非它大於視圖，此時會均勻縮小以使其包含在視圖中。

- `center`: 在兩個維度上將圖像置中於視圖。若圖像大於視圖，則均勻縮小以使其包含在視圖中。
...
當 `resizeMethod` 設為 `resize` 時，目標尺寸會乘以此值。`scale` 方法用於執行剩餘的調整。默認值 `1.0` 表示點陣圖尺寸設計為符合目標尺寸。大於 `1.0` 的乘數會將調整選項設為大於目標尺寸，生成的點陣圖會從硬體尺寸縮小。

此屬性在目標尺寸非常小且來源圖像明顯較大時最有用。`resize` 方法會進行降採樣，來源與目標圖像尺寸間會損失大量畫質，常導致圖像模糊。透過使用乘數，解碼後的圖像會略大於目標尺寸但小於來源圖像（若來源圖像足夠大）。這允許混疊偽影透過對放大圖像的縮放操作產生虛擬畫質。

若來源圖像尺寸為 200x200 且目標尺寸為 24x24，`resizeMultiplier` 設為 `2.0` 會指示 Fresco 將圖像降採樣至 48x48。Fresco 會選擇最接近的 2 的冪次方（即 50x50）並將圖像解碼為該尺寸的點陣圖。沒有乘數時，最接近的 2 的冪次方會是 25x25。生成的圖像會由系統縮小。
...
此屬性亦可包含多個遠端 URL，並同時指定其寬度、高度及可能的縮放比例/其他 URI 參數。原生端會根據圖片容器的測量尺寸，自動選擇最合適的 `uri` 顯示。可添加 `cache` 屬性來控制網路請求與本地快取的互動方式（詳見[圖片快取控制](images#cache-control)）。
...
代表以逗號分隔的候選圖片來源清單的字串。每個圖片來源包含圖片的 URL 和像素密度描述符。若未指定描述符，則默認為 `1x`。
...
在顯示圖片前獲取其寬度和高度（以像素為單位）。若圖片無法找到或下載失敗，此方法可能失敗。

為獲取圖片尺寸，可能需要先加載或下載圖片，之後會將其緩存。理論上您可用此方法預載圖片，但此方法並非為此優化，未來可能以不完全加載/下載圖片數據的方式實現。正式的圖片預載 API 將另行提供。
...
為了取得圖片的尺寸，可能需要先載入或下載圖片，之後會將其緩存。這意味著原則上可以使用此方法來預載圖片，但此方法並非為此目的而優化，未來可能會以不完全載入/下載圖片數據的方式實現。預載圖片的正式支援方式將作為獨立的API提供。
...
預取遠端圖片以供後續使用，方法是將其下載到磁碟緩存中。返回一個解析為布林值的Promise。
...
執行緩存查詢。返回一個Promise，解析為從URL到緩存狀態的映射，例如「disk」、「memory」或「disk/memory」。如果請求的URL不在映射中，表示它不在緩存中。
...
將資產引用解析為一個物件，該物件具有屬性`uri`、`scale`、`width`和`height`。
...
- `default`：使用原生平台的默認策略。
- `reload`：URL的數據將從原始來源載入。不應使用現有的緩存數據來滿足URL載入請求。
- `force-cache`：將使用現有的緩存數據來滿足請求，無論其存儲時間或過期日期如何。如果緩存中沒有對應請求的數據，則從原始來源載入數據。
- `only-if-cached`：將使用現有的緩存數據來滿足請求，無論其存儲時間或過期日期如何。如果緩存中沒有對應URL載入請求的數據，則不會嘗試從原始來源載入數據，並且載入被視為失敗。
