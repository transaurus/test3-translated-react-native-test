...
`runAfterInteractions` 接受一個普通回調函數，或一個帶有返回 `Promise` 的 `gen` 方法的 `PromiseTask` 對象。如果提供 `PromiseTask`，則會完全解析（包括通過 `runAfterInteractions` 安排更多任務的異步依賴項），然後才開始處理可能較早同步排隊的下一個任務。
