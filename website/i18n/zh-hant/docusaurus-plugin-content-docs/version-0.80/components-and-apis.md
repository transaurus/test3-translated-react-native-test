...
React Native 提供了許多內建的[核心組件](intro-react-native-components)，可直接在應用程式中使用。您可以在左側邊欄（或窄螢幕上方的選單）中找到所有組件。如果不確定從何開始，請參考以下分類：
...
與更通用的 [`ScrollView`](./scrollview) 不同，以下列表視圖組件僅渲染當前顯示在螢幕上的元素。這使其成為顯示長數據列表的高效能選擇。
...
這些組件可能對特定應用程式有所幫助。如需完整的組件和 API 列表，請查看左側邊欄（或窄螢幕上方的選單）。
