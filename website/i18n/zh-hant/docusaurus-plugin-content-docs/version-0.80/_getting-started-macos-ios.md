...
您還需要安裝 Xcode 命令行工具。開啟 Xcode 後，從選單選擇 **Settings... (或 Preferences...)**，進入 Locations 面板，從 Command Line Tools 下拉選單中選擇最新版本進行安裝。

![Xcode Command Line Tools](/docs/assets/GettingStartedXcodeCommandLineTools.png)

#### 在 Xcode 中安裝 iOS 模擬器

要安裝模擬器，請開啟 **Xcode > Settings... (或 Preferences...)** 並選擇 **Platforms (或 Components)** 標籤頁。選擇與您想使用的 iOS 版本相對應的模擬器。
...
從 React Native 0.69 版本開始，可透過模板提供的 `.xcode.env` 檔案設定 Xcode 環境。

`.xcode.env` 檔案包含一個環境變數，用於在 `NODE_BINARY` 變數中導出 `node` 執行檔的路徑。
這是 **建議做法** ，可將建置基礎設施與系統版本的 `node` 解耦。若您的路徑或 node 版本管理工具與預設值不同，應自行調整此變數。

此外，您還可以在建置腳本階段添加任何其他環境變數並載入 `.xcode.env` 檔案。若需執行需要特定環境的腳本，這是 **建議做法** ：可將建置階段與特定環境解耦。
...
[ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"  # 此行用於載入 nvm
```
