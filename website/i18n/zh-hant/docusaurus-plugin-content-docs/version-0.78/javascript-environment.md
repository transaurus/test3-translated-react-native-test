...
- 多數情況下，React Native 會使用專為其優化的開源引擎 [Hermes](hermes)
- 若停用 Hermes，則會改用 Safari 的 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore) 引擎（注意 iOS 上的 JavaScriptCore 因應用程式記憶體寫入限制無法使用 JIT 編譯）
- 使用 Chrome 調試時，所有 JavaScript 程式碼會在 Chrome 的 V8 引擎中執行，並透過 WebSocket 與原生程式碼通訊

雖然這些環境高度相似，但仍可能出現細微差異，建議避免依賴特定運行時的獨有特性

## JavaScript 語法轉換器

語法轉換器讓您能使用最新的 JavaScript 語法，無需等待所有直譯器支援

React Native 內建 [Babel 編譯器](https://babeljs.io)，詳見 [Babel 文件](https://babeljs.io/docs/plugins/#transform-plugins)了解支援的轉換功能

完整轉換規則清單請參閱 [@react-native/babel-preset](https://github.com/facebook/react-native/tree/main/packages/react-native-babel-preset)
...
下列標準函式在所有支援的 JavaScript 運行時皆可用
