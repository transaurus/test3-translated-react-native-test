...
React 元件封裝了平台專用的 `DrawerLayout`（僅限 Android）。抽屜（通常用於導航）透過 `renderNavigationView` 渲染，直接子元件則是主視圖（放置內容的位置）。導航視圖初始狀態不會顯示在螢幕上，但可從 `drawerPosition` 屬性指定的視窗邊緣拉出，其寬度可由 `drawerWidth` 屬性設定。
