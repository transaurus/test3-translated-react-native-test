...
無障礙操作允許輔助技術以程式化方式觸發元件行為。`accessibilityActions` 屬性應包含一組操作物件，每個物件需定義操作名稱與標籤。
...
當使用者觸發無障礙操作時呼叫的函式，參數為包含操作名稱的事件物件。
...
指定使用者向下導航時接收焦點的下一視圖。參閱 [Android 文件](https://developer.android.com/reference/android/view/View.html#attr_android:nextFocusDown)。
