...
狀態列屬性變化之間的過渡是否應動畫化。支持 `backgroundColor`、`barStyle` 和 `hidden` 屬性。
...
狀態列是否透明。當 translucent 設置為 `true` 時，應用程式將在狀態列下繪製。這在使用半透明狀態列顏色時很有用。

:::warning
由於 Android 15 引入了邊到邊強制執行，在 API 級別 35 中設置狀態列為透明已被棄用，設置它將不會產生效果。您可以在此處閱讀更多關於我們的[邊到邊建議](https://github.com/react-native-community/discussions-and-proposals/discussions/827)。
:::
