...
`TouchableWithoutFeedback` 僅支援單一子元件。若需多個子元件，請將其包裹在 View 中。需特別注意，`TouchableWithoutFeedback` 的工作原理是克隆其子元件並套用響應屬性，因此任何中介元件都必須將這些屬性傳遞至底層的 React Native 元件。
...
此值決定當色彩反轉功能開啟時，此視圖是否應被反轉。設為 `true` 時，即使色彩反轉開啟，視圖也不會被反轉。
...
當設為 `true` 時，表示該視圖為無障礙元素。預設情況下，所有可觸碰元素均具無障礙功能。
...
覆寫螢幕閱讀器在使用者與元素互動時朗讀的文字。預設情況下，標籤由遍歷所有子元件並串接以空格分隔的 `Text` 節點構成。
...
此值指定螢幕閱讀器在使用者與元素互動時應使用的語言。其格式應符合 [BCP 47 規範](https://www.rfc-editor.org/info/bcp47)。
...
當無障礙標籤無法明確表達操作結果時，無障礙提示能幫助使用者理解對無障礙元素執行動作後的預期結果。
...
無障礙操作允許輔助技術以程式化方式觸發元件的操作。`accessibilityActions`屬性應包含操作對象列表，每個操作對象需包含名稱(name)與標籤(label)欄位。
...
表示可勾選元素的狀態。此欄位可接受布林值或"mixed"字串（代表混合狀態核取方塊）。
