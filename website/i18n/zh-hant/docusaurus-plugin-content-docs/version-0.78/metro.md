...
在 React Native 中，您的 Metro 設定應擴展自 [`@react-native/metro-config`](https://www.npmjs.com/package/@react-native/metro-config) 或 [`@expo/metro-config`](https://www.npmjs.com/package/@expo/metro-config)。這些套件包含了建構與執行 React Native 應用程式所需的必要預設值。
...
**但是！**，我們建議在覆寫這些設定值時進行複製與編輯——將真實來源放在您的設定檔中。
