...
> 雖然尺寸資訊可以立即取得，但它們可能會發生變化（例如由於設備旋轉、折疊式設備等），因此任何依賴這些常數的渲染邏輯或樣式都應該在每次渲染時調用此函數，而不是快取值（例如，使用內聯樣式而不是在 `StyleSheet` 中設定值）。
...
> 對於 Android 來說，`window` 的尺寸將排除 `狀態欄`（如果不透明）和 `底部導航欄` 所佔用的空間
