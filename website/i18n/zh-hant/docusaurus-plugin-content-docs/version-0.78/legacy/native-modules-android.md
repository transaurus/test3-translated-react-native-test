...
在本指南中，您將建立一個名為 `CalendarModule` 的原生模組，該模組可讓您從 JavaScript 存取 Android 的日曆 API。最終，您將能夠從 JavaScript 調用 `CalendarModule.createCalendarEvent('晚餐派對', '我家');`，從而觸發 Java/Kotlin 方法來建立日曆事件。
...
第一步是在 `android/app/src/main/java/com/您的應用程式名稱/` 資料夾內建立 (`CalendarModule.java` 或 `CalendarModule.kt`) Java/Kotlin 檔案（Kotlin 和 Java 的資料夾路徑相同）。該 Java/Kotlin 檔案將包含您的原生模組 Java/Kotlin 類別。
...
在方法中添加一個除錯日誌，以確認在從應用程式調用時它已被調用。以下是您如何從 Android util 套件導入和使用 [Log](https://developer.android.com/reference/android/util/Log) 類別的範例：
...
React Native 調用 `ReactPackage` 上的 `createNativeModules()` 方法來獲取要註冊的原生模組列表。對於 Android，如果模組未在 `createNativeModules` 中實例化並返回，則無法從 JavaScript 中使用。
...
至此，您已為 Android 原生模組搭建好基礎架構。請透過 JavaScript 存取原生模組並呼叫其導出方法來進行測試。
...
現在您已擁有可用的 CalendarModule 原生模組，可以呼叫原生方法 `createCalendarEvent()`。以下將其新增至 `NewModuleButton` 的 `onPress()` 方法中：
...
當您遵循本指南迭代開發原生模組時，需透過原生建置來讓 JavaScript 端取得最新變更。這是因為您編寫的程式碼位於應用的原生部分。雖然 React Native 的 Metro 打包工具能監聽 JavaScript 變更並即時重建，但對原生程式碼無效。因此若要測試最新的原生修改，必須使用上述指令重新建置。

### 重點回顧 ✨

現在您應能在應用程式中呼叫原生模組的 `createCalendarEvent()` 方法。在我們的範例中，透過點擊 `NewModuleButton` 觸發此操作。您可透過檢視 `createCalendarEvent()` 方法中設定的日誌來確認，請依照[這些步驟](https://developer.android.com/studio/debug/am-logcat.html)查看應用程式的 ADB 日誌。搜尋您設定的 `Log.d` 訊息（範例中為「Create event called with name: testName and location: testLocation」），每次呼叫原生模組方法時都應看到該日誌訊息。
...
至此您已完成 Android 原生模組的建立，並從 React Native 應用的 JavaScript 端呼叫其原生方法。您可以繼續閱讀以了解更多內容，例如原生模組方法支援的參數類型，以及如何設定回調函式與 Promise。
...
為避免模組使用者每次存取時都需如此操作，可為原生模組建立 JavaScript 封裝層。建立名為 `CalendarModule.js` 的新 JavaScript 檔案，內容如下：
...
此 JavaScript 檔案也是新增 JavaScript 端功能的理想位置。例如若您使用 TypeScript 等型別系統，可在此為原生模組添加型別註解。雖然 React Native 尚未支援 Native 到 JS 的型別安全，但所有 JS 程式碼都將具備型別安全。此做法也能讓您未來更容易遷移到型別安全的原生模組。以下是為 CalendarModule 添加型別安全的範例：
...
當 JavaScript 呼叫原生模組方法時，React Native 會將參數從 JS 物件轉換為對應的 Java/Kotlin 物件。舉例來說，如果 Java 原生模組方法接收一個 double 類型，在 JS 中呼叫時需傳入數字。React Native 會自動處理類型轉換。以下是原生模組方法支援的參數類型及其對應的 JavaScript 等效類型清單：
...
若需使用清單未列出的參數類型，您需自行處理轉換。例如在 Android 中，`Date` 類型並未預設支援轉換。您可於原生方法內自行處理 `Date` 類型的轉換，如下所示：
...
要建立帶回調的原生模組方法，需先導入 `Callback` 介面，並在方法參數中添加 `Callback` 類型參數。需注意 TurboModules 將解除以下兩項回調限制：1) 函數參數中最多只能有兩個回調（successCallback 和 failureCallback）；2) 若方法的最後一個參數是函數，會被視為 successCallback，倒數第二個函數參數則視為 failureCallback。
...
重要細節：原生模組方法只能呼叫一次回調（成功或失敗擇一），且每個回調最多觸發一次。但原生模組可儲存回調並延後觸發。
...
接著在 JavaScript 中，您可以為錯誤和成功回應分別添加回調：
...
如果您想從使用 `startActivityForResult` 啟動的活動中取得結果，您需要監聽 `onActivityResult`。為此，您必須擴展 `BaseActivityEventListener` 或實現 `ActivityEventListener`。前者更受推薦，因為它對 API 變更更具彈性。接著，您需要在模組的構造函數中註冊監聽器，如下所示：
...
讓我們實現一個基本的圖片選擇器來演示這一點。圖片選擇器將向 JavaScript 公開 `pickImage` 方法，呼叫時會返回圖片的路径。
...
截至目前，在 Android 上，所有原生模組的非同步方法都在單一執行緒上執行。原生模組不應對其被呼叫的執行緒做出任何假設，因為目前的分配方式未來可能會變更。如果需要進行阻塞呼叫，繁重的工作應分派到內部管理的工作執行緒，並從那裡分發任何回呼。