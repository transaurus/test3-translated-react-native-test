...
在以下範例中，紅色、橘色和綠色視圖都是容器視圖的子元素，且容器設定了 `flex: 1`。紅色視圖使用 `flex: 1`，橘色視圖使用 `flex: 2`，綠色視圖使用 `flex: 3`。**1+2+3 = 6**，這意味著紅色視圖將獲得 `1/6` 的空間，橘色視圖獲得 `2/6`，綠色視圖獲得 `3/6`。
...
[`flexDirection`](layout-props#flexdirection) 控制節點子元素的排列方向，這也被稱為主軸。交叉軸則是與主軸垂直的軸，或是換行線的排列方向。
...
佈局 [`direction`](layout-props#direction) 指定了子元素和文字在層次結構中的排列方向。佈局方向也會影響 `start` 和 `end` 所指向的邊緣。預設情況下，React Native 使用 LTR（從左到右）佈局方向。在此模式下，`start` 指向左側，`end` 指向右側。
...
- `space-around` 在主軸上均勻分布子元素，剩餘空間環繞子元素分配。與 `space-between` 相比，`space-around` 會在首個子元素前和最後子元素後分配空間。

- `space-evenly` 在主軸上完全均勻分布子元素。相鄰子元素間距、主軸起始端與首個子元素間距、主軸末端與最後子元素間距均完全相同。
...
- `baseline` 使子元素沿共同基線對齊。可為個別子元素設定其父元素的參考基線。

:::info
要使 `stretch` 生效，子元素在次要軸上不能有固定尺寸。下例中，移除子元素的 `width: 50` 後，`alignItems: stretch` 才會產生效果。
:::
...
- `space-between` 在交叉軸上均勻分布換行內容，剩餘空間分配於行間。

- `space-around` 在交叉軸上均勻分布換行內容，剩餘空間環繞各行分配。容器兩端的空間為行間距的一半。

- `space-evenly` 在交叉軸上完全均勻分布換行內容，所有間距大小相同。
...
[`flexWrap`](layout-props#flexwrap) 屬性設置在容器上，用於控制當子元素沿主軸溢出容器尺寸時的行為。預設情況下，子元素會被強制排列在單一行上（這可能會縮小元素尺寸）。如果允許換行，則會根據需要沿主軸將項目換行至多行顯示。
...
- [`flexBasis`](layout-props#flexbasis) 是一種與軸向無關的方式，用於設定項目沿主軸的預設尺寸。設定子元素的 `flexBasis` 類似於在父容器為 `flexDirection: row` 時設定該子元素的 `width`，或在父容器為 `flexDirection: column` 時設定其 `height`。項目的 `flexBasis` 是該項目的預設尺寸，即在任何 `flexGrow` 和 `flexShrink` 計算之前的尺寸。

- [`flexGrow`](layout-props#flexgrow) 描述容器內的主軸空間應如何在其子元素之間分配。在佈局子元素後，容器會根據子元素指定的 flex grow 值分配剩餘空間。

  `flexGrow` 接受任何大於或等於 0 的浮點數，預設值為 0。容器會按照子元素的 `flexGrow` 值加權分配剩餘空間。

- [`flexShrink`](layout-props#flexshrink) 描述當子元素的總尺寸在主軸上超出容器尺寸時，應如何沿主軸縮小子元素。`flexShrink` 與 `flexGrow` 非常相似，可以將任何溢出的尺寸視為負剩餘空間來理解。這兩個屬性也能很好地協同工作，允許子元素根據需要伸展或收縮。

  `flexShrink` 接受任何大於或等於 0 的浮點數，預設值為 0（在網頁上預設為 1）。容器會按照子元素的 `flexShrink` 值加權縮小子元素。
...
- `absolute` 當元素採用絕對定位時，不會參與正常的版面流動。而是獨立於其兄弟元素進行布局。其位置由 `top`、`right`、`bottom` 和 `left` 的值決定，這些值會將元素相對於其包含區塊（containing block）進行定位。

- `static` 當元素採用靜態定位時，會依照正常的版面流動進行定位，並忽略 `top`、`right`、`bottom` 和 `left` 的值。此 `position` 設定還會導致元素不會為其絕對定位的子代形成包含區塊，除非存在其他優先的樣式屬性（例如 `transform`）。這允許 `absolute` 元素定位到非其父元素的物件上。請注意，**`static` 僅在新架構（New Architecture）中可用**。
...
查看互動式的 [yoga playground](https://www.yogalayout.dev/playground)，您可以透過它更深入理解 flexbox。

我們已經涵蓋了基礎知識，但您可能還需要許多其他樣式來進行布局。控制布局的完整屬性列表記錄在 [這裡](./layout-props.md)。
