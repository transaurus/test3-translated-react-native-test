...
每當我們發布新版的 React Native 時，都會為您構建對應的 Hermes 版本。這確保您使用的 Hermes 版本與 React Native 完全相容。
...
若您近期新建了應用程式，應能在歡迎畫面中確認 Hermes 是否啟用：

![在 AwesomeProject 中查看 JS 引擎狀態的位置](/docs/assets/HermesApp.jpg)

JavaScript 中會存在一個全域變數 `HermesInternal`，可用於驗證是否使用 Hermes：
...
請確認您使用的是 `.hbc` 檔案，並依下述方式進行前後效能比對。
:::

要驗證 Hermes 的效益，請嘗試建置應用的正式版本進行比較。例如在專案根目錄執行：
...
這將在建置時將 JavaScript 編譯為 Hermes 位元組碼，從而提升裝置上的應用啟動速度。
