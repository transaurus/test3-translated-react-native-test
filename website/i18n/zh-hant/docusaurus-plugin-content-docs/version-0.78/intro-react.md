...
> 這是導出組件的多種方式之一。此類導出方式與 Snack Player 兼容性良好。但根據應用的文件結構，可能需要採用其他約定方式。[JavaScript 導入導出速查表](https://medium.com/dailyjs/javascript-module-cheatsheet-7bd474f1d829)可提供幫助。
...
大多數 React Native 的核心元件也可以透過 props 客製化。舉例來說，使用 [`Image`](image) 時，你可以傳遞名為 [`source`](image#source) 的 prop 來定義它顯示的圖片：
...
> 注意 `style` 的寬度和高度周圍的雙大括號 `{{ }}`。在 JSX 中，JavaScript 值使用 `{}` 來引用。這在傳遞非字串的 props 時很方便，例如陣列或數字：`<Cat food={["fish", "kibble"]} age={2} />`。然而，JS 物件**同樣**使用大括號表示：`{width: 200, height: 200}`。因此，要在 JSX 中傳遞 JS 物件，你必須**再包一層**大括號：`{{width: 200, height: 200}}`

你可以透過 props 和核心元件 [`Text`](text)、[`Image`](image) 和 [`View`](view) 建構許多東西！但要建立互動式功能，你需要狀態。

## State

你可以將 props 視為用來配置元件渲染的參數，而**state** 則是元件的個人資料儲存空間。State 適用於處理隨時間變化的資料或來自使用者互動的資料。State 賦予你的元件記憶力！

> 一般來說，使用 props 來配置元件渲染時的樣式。使用 state 來追蹤任何你預期會隨時間變化的元件資料。

以下範例發生在一間貓咪咖啡館，兩隻飢餓的貓咪等待被餵食。牠們的飢餓狀態（我們預期會隨時間變化，與牠們的名字不同）被儲存為 state。要餵食貓咪，請按下牠們的按鈕——這會更新牠們的狀態。

你可以透過呼叫 [React 的 `useState` Hook](https://react.dev/learn/state-a-components-memory) 來為元件添加 state。Hook 是一種函數，讓你能「勾入」React 的功能。例如，`useState` 是一個 Hook，讓你能為函數元件添加 state。你可以在 [React 文件](https://react.dev/reference/react) 中了解更多關於其他種類的 Hook。
...
接著，在元件函數內部呼叫 `useState` 來宣告元件的 state。在此範例中，`useState` 建立了一個 `isHungry` 狀態變數：
...
- 建立一個帶有初始值的「狀態變數」——在此範例中，狀態變數是 `isHungry`，其初始值為 `true`
- 建立一個用來設定該狀態變數值的函數——`setIsHungry`
...
> 你可能注意到雖然 `isHungry` 是 [const](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Statements/const)，但它似乎可以被重新賦值！實際情況是當像 `setIsHungry` 這樣的狀態設置函數被呼叫時，其元件會重新渲染。此時 `Cat` 函數會再次執行——而這次 `useState` 會給我們 `isHungry` 的下一個值。
