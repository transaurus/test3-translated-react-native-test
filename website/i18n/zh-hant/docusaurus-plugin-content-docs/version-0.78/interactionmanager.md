...
InteractionManager 允許在互動/動畫完成後排程長時間運行的任務，這能確保 JavaScript 動畫流暢運行。
...
InteractionManager 也允許應用程式在動畫開始時建立互動「控制代碼」，並在完成時清除：
...
`runAfterInteractions` 接受普通回調函數或帶有返回 `Promise` 的 `gen` 方法的 `PromiseTask` 物件。若提供 `PromiseTask`，則會完全解析（包括透過 `runAfterInteractions` 排程更多任務的非同步依賴項）後，才開始執行可能先前同步排隊的下一個任務。

預設情況下，排隊任務會在一個 `setImmediate` 批次中一起執行。若呼叫 `setDeadline` 並傳入正數，則任務僅會在截止時間（以 JavaScript 事件循環運行時間為準）接近前執行，此時會透過 setTimeout 讓出執行權，允許觸控等事件開始互動並阻止排隊任務執行，提升應用程式回應速度。
...
傳入正數會使用 setTimeout 在事件循環運行時間達到截止值後排程任務，否則所有任務會在一個 setImmediate 批次中執行（預設）。