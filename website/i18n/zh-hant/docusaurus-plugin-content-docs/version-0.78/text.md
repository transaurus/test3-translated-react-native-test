...
Android 和 iOS 都允許您通過用特定格式（如粗體或彩色文字）標註字符串的範圍來顯示格式化文字（iOS 上的 `NSAttributedString`，Android 上的 `SpannableString`）。實際上，這非常繁瑣。對於 React Native，我們決定使用網頁範式來實現相同的效果，即可以嵌套文字。
...
`<Text>` 元素在佈局方面是獨特的：內部的一切不再使用 Flexbox 佈局，而是使用文字佈局。這意味著 `<Text>` 內的元素不再是矩形，而是在看到行尾時換行。
...
- (開發者) React 元件設計時考慮了強隔離性：您應該能夠將元件放置在應用程序的任何位置，並相信只要屬性相同，它將以相同的方式外觀和行為。可以從屬性外部繼承的文字屬性將破壞這種隔離性。
...
設定在 Android API 等級 23+ 上確定斷字時使用的自動斷字頻率。
...
決定文字元素中哪些類型的資料會被轉換為可點擊的URL。預設不偵測任何資料類型。
...
此視圖是否要「聲明」觸控響應？當該視圖不是響應者時，每次觸控移動都會調用此函數。
...
響應者權限已從該視圖移除。可能因調用 `onResponderTerminationRequest` 後被其他視圖取得，也可能被作業系統未經詢問直接取得（例如 iOS 的控制中心/通知中心）
...
當其他 `View` 想要成為觸控回應者並要求此 `View` 釋放其回應者權限時觸發。返回 `true` 表示允許釋放。
...
若父層 `View` 欲阻止子層 `View` 在觸控開始時成為回應者，應實作此處理函式並返回 `true`。
...
當滾動視圖被禁用時，此屬性定義觸控點可偏離按鈕多遠才會停用按鈕。一旦停用，將觸控點移回即可重新啟用按鈕。在滾動視圖禁用狀態下反覆測試此行為。建議傳入常數以減少記憶體配置。
...
設為 `true` 時，文字被按下時不會有視覺變化。預設情況下，按下時會顯示灰色橢圓形高亮效果。
