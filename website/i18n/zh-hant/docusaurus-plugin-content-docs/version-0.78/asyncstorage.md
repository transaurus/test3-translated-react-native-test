...
在 iOS 上，`AsyncStorage` 底層由原生程式碼實現，小量值會儲存於序列化字典，較大值則存於獨立檔案。Android 端會根據可用性採用 [RocksDB](https://rocksdb.org/) 或 SQLite。

`AsyncStorage` 的 JavaScript 層為門面模式，提供清晰的 JS API、真實的 `Error` 物件及非批次操作方法。每個 API 方法均回傳 `Promise` 物件。
...
為 `key` 設定值並於完成時觸發回調。回傳 `Promise` 物件。
...
合併既有 `key` 值與輸入值（假設兩者均為 JSON 字串）。回傳 `Promise` 物件。
...
清除 _所有_ 客戶端、函式庫等的 `AsyncStorage` 資料。通常不應直接呼叫此方法，建議改用 `removeItem` 或 `multiRemove` 僅清除應用程式自身的鍵值。回傳 `Promise` 物件。
...
取得應用程式所有已知的鍵值（含所有呼叫者與函式庫）。回傳 `Promise` 物件。
...
使用此方法批次儲存多個鍵值對。操作完成時將收到單一包含錯誤的回調：
