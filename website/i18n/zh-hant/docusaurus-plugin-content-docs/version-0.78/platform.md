...
返回一個布林值，表示裝置是否為Apple Vision。_若您使用[Apple Vision Pro (專為iPad設計)](https://developer.apple.com/documentation/visionos/checking-whether-your-app-is-compatible-with-visionos)，`isVision`將為`false`但`isPad`將為`true`_
...
select方法會返回最適合當前運行平台的值。也就是說，如果您在手機上運行，`android`和`ios`鍵將優先。如果未指定這些鍵，則會使用`native`鍵，然後是`default`鍵。
