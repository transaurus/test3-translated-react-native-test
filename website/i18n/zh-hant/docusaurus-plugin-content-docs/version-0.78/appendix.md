...
- **Spec** - 用於描述 Turbo Native 模組或 Fabric Native 元件的 API 之 TypeScript 或 Flow 程式碼。由 **Codegen** 用於生成樣板程式碼。

- **原生模組 (Native Modules)** - 不包含使用者介面(UI)的原生函式庫。例如持久化儲存、通知、網路事件等。這些可透過函式與物件形式供您的 JavaScript 應用程式碼存取。
- **原生元件 (Native Component)** - 可透過 React 元件形式供您的應用程式 JavaScript 碼使用的原生平台視圖。
...
<b>[1]</b> 我們強烈建議使用物件字面量(Object literals)而非物件(Object)。
