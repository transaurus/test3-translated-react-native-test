...
`style` 屬性可以是一個普通的 JavaScript 物件。這是我們在範例代碼中通常使用的形式。您也可以傳遞一個樣式陣列 - 陣列中的最後一個樣式具有優先權，因此您可以使用此方式來繼承樣式。
...
一個常見的模式是讓您的元件接受 `style` 屬性，該屬性又用於為子元件設定樣式。您可以使用此方式讓樣式「層疊」，就像在 CSS 中一樣。

有更多方法可以自訂文字樣式。查看[文字元件參考](text.md)以獲取完整列表。
