...
- 為樣式命名有助於為渲染函式中的底層元件增添意義，並促進重用。
- 在多數整合開發環境中，使用 `StyleSheet.create()` 會提供靜態類型檢查與建議，協助您撰寫有效的樣式。
...
將兩種樣式組合，使 `style2` 覆蓋 `style1` 中的任何樣式。若任一樣式為假值，則直接返回另一個樣式而不分配陣列，節省記憶體分配並維持 PureComponent 檢查的引用相等性。
...
設定一個函式用於預處理樣式屬性值。此功能內部用於處理顏色與變形值。除非您確知自己在做什麼且已窮盡其他選項，否則不應使用此功能。
...
一個非常常見的模式是創建具有絕對定位和零定位的覆蓋層（`position: 'absolute', left: 0, right: 0, top: 0, bottom: 0`），因此可使用 `absoluteFill` 來方便地減少這些重複樣式的冗餘。如果需要，`absoluteFill` 可用於在 StyleSheet 中創建自定義條目，例如：
