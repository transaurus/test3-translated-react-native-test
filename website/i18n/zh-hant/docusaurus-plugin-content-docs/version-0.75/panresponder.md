...
`PanResponder` 將多個觸控點協調為單一手勢。它使單點觸控手勢能抵抗額外觸控點的干擾，並可用於識別基本的多點觸控手勢。
...
- `stateID` - 手勢狀態的 ID，只要螢幕上至少有一個觸控點就會持續存在
- `moveX` - 最近移動觸控點的最新螢幕座標 X 值
- `moveY` - 最近移動觸控點的最新螢幕座標 Y 值
- `x0` - 響應器授予時的螢幕座標 X 值
- `y0` - 響應器授予時的螢幕座標 Y 值
...
`PanResponder` 可與 `Animated` API 協同工作，幫助構建 UI 中的複雜手勢。以下範例包含一個動畫 `View` 組件，可以在螢幕上自由拖曳：
...
使用 `onStartShould*` 回調時需格外謹慎。這些回調僅反映冒泡/捕獲到節點的開始/結束事件所更新的 `gestureState`。一旦節點成為響應者，您可以確信每個開始/結束事件都會被手勢處理，且 `gestureState` 會相應更新。（numberActiveTouches）可能不完全準確，除非您是響應者。