...
- 使用 Chrome 除錯時，所有 JavaScript 程式碼會在 Chrome 瀏覽器內執行，並透過 WebSocket 與原生程式碼通訊。Chrome 使用 [V8](https://v8.dev/) 作為 JavaScript 引擎。

雖然這些環境非常相似，但仍可能遇到某些不一致的情況。建議避免依賴特定執行環境的獨有特性。

## JavaScript 語法轉換器

語法轉換器讓開發者能使用最新的 JavaScript 語法，無需等待所有直譯器支援，大幅提升編碼體驗。
...
完整的 React Native 轉換器清單可查閱 [@react-native/babel-preset](https://github.com/facebook/react-native/tree/main/packages/react-native-babel-preset)。
