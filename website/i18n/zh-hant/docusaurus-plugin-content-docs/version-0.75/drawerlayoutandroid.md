...
封裝平台原生 `DrawerLayout` 的 React 元件（僅限 Android）。抽屜式導航欄透過 `renderNavigationView` 渲染，主視圖內容則直接作為子元件。導航視圖初始狀態為隱藏，可透過 `drawerPosition` 屬性指定從螢幕哪一側滑出，並以 `drawerWidth` 屬性設定其寬度。
...
設定抽屜背景色，預設值為 `white`。若需設定透明度請使用 rgba 格式，例如：
