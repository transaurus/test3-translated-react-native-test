...
注意包圍 `{pic}` 的大括號——這些符號將變數 `pic` 嵌入到 JSX 中。你可以在 JSX 的大括號內放入任何 JavaScript 表達式。
...
使用 `name` 作為 prop 讓我們可以自訂 `Greeting` 元件，因此我們可以重複使用該元件來顯示每個問候語。這個範例也像[核心元件](intro-react-native-components)一樣在 JSX 中使用 `Greeting` 元件。能夠這樣做正是 React 的強大之處——如果你發現自己希望使用一組不同的 UI 基本元件，你可以創造新的元件。
