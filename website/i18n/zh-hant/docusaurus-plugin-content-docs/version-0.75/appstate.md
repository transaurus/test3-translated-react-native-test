...
AppState 常被用於處理推送通知時，判斷意圖並決定適當行為。
...
要查看當前狀態，您可以檢查 `AppState.currentState`，該值會保持最新狀態。但在啟動時，`currentState` 會暫時為 null，直到 `AppState` 透過橋接取得狀態。
...
此範例永遠只會顯示「目前狀態為：active」，因為應用程式只有在 `active` 狀態時對使用者可見，而 null 狀態只會短暫出現。若要實驗此程式碼，建議使用實體裝置而非嵌入式預覽。
...
設定當 AppState 上指定事件類型發生時要呼叫的函式。有效的 `eventType` 值請參閱[上方列表](#events)。回傳 `EventSubscription`。
