...
iOS 專用：設定開關背景自訂顏色。當開關值為 `false` 或開關被禁用時（此時開關呈半透明狀），可看到此背景色。
...
_iOS 注意_：當開關值為 `false` 時，軌道會向邊界收縮。若要調整收縮後露出的背景色，請使用 [`ios_backgroundColor`](switch.md#ios_backgroundColor)。
