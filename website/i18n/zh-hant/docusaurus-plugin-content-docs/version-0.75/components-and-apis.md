...
您不僅限於使用 React Native 內建的元件與 API。React Native 擁有數千名開發者組成的社群。如需尋找特定功能的函式庫，請參閱[尋找函式庫指南](libraries#finding-libraries)。
...
與通用型的 [`ScrollView`](./scrollview) 不同，以下列表視圖元件僅渲染當前顯示在畫面上的元素，這使其成為顯示長數據列表的高效能選擇。
...
這些元件可能對特定應用程式有所助益。如需完整的元件與 API 清單，請查看左側邊欄（或窄螢幕上方的選單）。
