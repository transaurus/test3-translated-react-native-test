...
若此按鈕不符合您的應用程式風格，可使用 [Pressable](pressable) 自行建構按鈕。設計靈感可參考 [Button 元件的原始碼](https://github.com/facebook/react-native/blob/main/packages/react-native/Libraries/Components/Button.js)。
...
允許輔助技術以程式化方式觸發元件操作。此屬性應包含動作物件列表，每個物件需定義動作名稱與標籤欄位。
...
當使用者觸發無障礙操作時呼叫，傳入參數為包含操作名稱的事件物件。
...
指定使用者向下導航時接收焦點的下一視圖。詳見 [Android 文件](https://developer.android.com/reference/android/view/View.html#attr_android:nextFocusDown)。
