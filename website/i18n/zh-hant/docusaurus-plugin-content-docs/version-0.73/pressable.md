...
手指並非最精確的工具，使用者常會意外觸發錯誤元素或錯過觸發區域。為此，`Pressable` 提供可選的 `HitRect`，可用於定義觸控可註冊的範圍距離包裝元素多遠。按壓可以在 `HitRect` 內的任何位置開始。

`PressRect` 允許按壓移動超出元素及其 `HitRect`，同時保持激活狀態並符合「按壓」資格——想像您按住按鈕時慢慢滑開手指的情況。
...
在 `onPressOut` 觸發前，此視圖外額外可被視為按壓動作的觸碰距離。
