...
用於驅動2D動畫（如平移手勢）的2D數值。其API幾乎與常規[`Animated.Value`](animatedvalue)相同，但採用多路復用技術。底層包含兩個常規的`Animated.Value`。
...
將偏移值合併到基礎值中，並將偏移量重置為零。最終輸出的數值保持不變。
...
將偏移值設為基礎值，並將基礎值重置為零。最終輸出的數值保持不變。
...
停止所有正在運行的動畫或追蹤。動畫停止後會以最終值調用`callback`，這對於將狀態更新至與動畫位置同步非常有用。
