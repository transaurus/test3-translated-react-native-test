...
回傳一個 Promise，當用戶選擇時間時，將以包含 `action`、`hour` (0-23)、`minute` (0-59) 的物件呼叫。若用戶關閉對話框，Promise 仍會以 `action` 為 `TimePickerAndroid.dismissedAction` 且其他鍵值為 undefined 的狀態解析。**務必**在讀取值之前檢查 `action`。
