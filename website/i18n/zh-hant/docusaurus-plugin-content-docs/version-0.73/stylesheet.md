...
> **警告：實驗性功能。** 可能頻繁發生破壞性變更且不會可靠地公告。整個功能可能被刪除，誰知道呢？使用風險自負。
...
設置用於預處理樣式屬性值的函式。此方法內部用於處理顏色和變換值。除非您確知自己在做什麼且已窮盡其他選項，否則不應使用。
...
絕對定位且四邊歸零（`position: 'absolute', left: 0, right: 0, top: 0, bottom: 0`）是創建疊層的常見模式。為方便起見並減少重複樣式，可使用 `absoluteFill`。您也可以用它在 StyleSheet 中創建自定義條目，例如：
...
此常數始終為整數像素值（使定義的線條看起來清晰），並會嘗試匹配底層平台細線的標準寬度。但您不應依賴其為固定尺寸，因為在不同平台和螢幕密度下，其計算方式可能不同。

如果您的模擬器縮放比例過低，hairline 寬度的線條可能無法顯示。