...
您可以在 https://easings.net/ 找到一些常見緩動函數的可視化示例。
...
提供一個三次貝塞爾曲線，等同於CSS過渡的`transition-timing-function`。

可視化三次貝塞爾曲線的實用工具可在 https://cubic-bezier.com/ 找到。
...
使任何緩動函數對稱。緩動函數將在前半段時間正向運行，然後在剩餘時間反向運行。