...
React 和 React Native 使用 **JSX**，這種語法允許在 JavaScript 中直接編寫元素：`<Text>Hello, I am your cat!</Text>`。React 文檔提供 [完整的 JSX 指南](https://react.dev/learn/writing-markup-with-jsx) 可供深入學習。由於 JSX 就是 JavaScript，可以在其中使用變量。此處聲明了貓的名字 `name`，並用花括號將其嵌入 `<Text>`。
...
> 注意 `style` 的寬度和高度周圍的雙大括號 `{{ }}`。在 JSX 中，JavaScript 值是用 `{}` 來引用的。這在傳遞非字串的 props 時非常方便，例如陣列或數字：`<Cat food={["fish", "kibble"]} age={2} />`。然而，JS 物件 **同樣** 是用大括號表示：`{width: 200, height: 200}`。因此，要在 JSX 中傳遞一個 JS 物件，你必須 **再用一對** 大括號將其包住：`{{width: 200, height: 200}}`

你可以透過 props 和核心元件 [`Text`](text)、[`Image`](image) 以及 [`View`](view) 來建構許多東西！但要建立互動式內容，你需要狀態。

## State

你可以將 props 視為用來配置元件渲染方式的參數，而 **state** 則像是元件的個人資料儲存空間。狀態適合用來處理隨時間變化的資料或來自使用者互動的資料。狀態賦予你的元件記憶力！
...
你可以透過呼叫 [React 的 `useState` Hook](https://react.dev/learn/state-a-components-memory) 來為元件添加狀態。Hook 是一種函式，讓你能夠「勾入」React 的功能。例如，`useState` 是一個 Hook，讓你能夠為函式元件添加狀態。你可以在 [React 文件](https://react.dev/reference/react) 中了解更多關於其他種類的 Hook。
...
然後，透過在元件函式中呼叫 `useState` 來宣告元件的狀態。在此範例中，`useState` 創建了一個 `isHungry` 狀態變數：
...
- 它創建一個用來設定該狀態變數值的函式——`setIsHungry`
...
> 你可能注意到雖然 `isHungry` 是 [const](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Statements/const)，但它似乎能被重新賦值！實際情況是當像 `setIsHungry` 這樣的狀態設定函數被呼叫時，其所屬元件會重新渲染。此時 `Cat` 函數會再次執行——而這次 `useState` 會給我們 `isHungry` 的下一個值。
...
現在你已了解 React 和 React Native 核心元件，讓我們更深入探討部分核心元件，從 [處理 `<TextInput>`](handling-text-input) 開始。