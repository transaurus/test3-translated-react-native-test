...
驅動動畫的標準值。一個 `Animated.Value` 可以同步驅動多個屬性，但同一時間只能由一個機制驅動。使用新的機制（例如啟動新動畫或調用 `setValue`）將會停止先前的所有驅動。
...
直接設定數值。這會停止所有在該值上運行的動畫，並更新所有綁定的屬性。
...
設定一個偏移量，該偏移量會疊加在無論是通過 `setValue`、動畫還是 `Animated.event` 設定的值之上。適用於補償平移手勢的起始點等情況。
...
取消註冊監聽器。參數 `id` 應與先前 `addListener()` 返回的標識符匹配。
