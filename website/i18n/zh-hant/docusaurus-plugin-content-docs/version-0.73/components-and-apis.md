...
React Native 提供了多種內建的[核心元件](intro-react-native-components)，可直接用於您的應用程式。您可以在左側邊欄（或窄螢幕上方的選單）中找到所有元件。如果不確定從何開始，請參考以下分類：
...
與通用型 [`ScrollView`](./scrollview) 不同，以下列表視圖元件僅渲染當前顯示在畫面上的元素，使其成為顯示長列表數據的高效能選擇。
...
這些元件可能對特定應用有所助益。如需完整的元件與 API 列表，請查看左側邊欄（或窄螢幕上方的選單）。
