...
在觸控手機上輸入文字是一項挑戰——螢幕小、軟體鍵盤。但根據您需要的資料類型，可以透過適當配置文字輸入欄位來簡化操作：
...
在手機上按鈕時很難非常精準。確保所有互動元素尺寸為 44x44 或更大。一種方法是為元素留足夠空間，`padding`、`minWidth` 和 `minHeight` 樣式值對此很有用。或者，您可以使用 [`hitSlop` 屬性](touchablewithoutfeedback.md#hitslop) 在不影響版面的情況下增加互動區域。以下是示範：
...
Android API 21+ 使用 Material Design 漣漪效果在使用者觸控螢幕互動區域時提供反饋。React Native 透過 [`TouchableNativeFeedback` 元件](touchablenativefeedback.md) 實現此效果。使用這種觸控效果而非透明度或高亮，通常會讓您的應用更符合平台風格。但需注意它不支援 iOS 或 Android API < 21，因此需要在 iOS 上回退使用其他 Touchable 元件。您可以使用像 [react-native-platform-touchable](https://github.com/react-community/react-native-platform-touchable) 這樣的函式庫來處理平台差異。
...
除非您使用 `Dimensions` API 且未處理方向變更，否則多種螢幕方向預設應能正常運作。如果不需支援多種螢幕方向，可將螢幕鎖定為直向或橫向。

在 iOS 上，於 Xcode 的 General 標籤和 Deployment Info 部分啟用您想支援的裝置方向（確保在變更時已從 Devices 選單中選擇 iPhone）。對於 Android，開啟 AndroidManifest.xml 文件並在 activity 元素中添加 `'android:screenOrientation="portrait"'` 鎖定為直向，或 `'android:screenOrientation="landscape"'` 鎖定為橫向。
