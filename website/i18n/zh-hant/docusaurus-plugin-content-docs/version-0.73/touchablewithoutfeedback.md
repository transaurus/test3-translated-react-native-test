...
`TouchableWithoutFeedback` 僅支持單個子元素。若需多個子組件，請將其包裹在 View 中。需特別注意，`TouchableWithoutFeedback` 的工作原理是克隆其子元素並應用響應屬性，因此任何中介組件都必須將這些屬性傳遞給底層的 React Native 組件。
...
當設為 `true` 時，表示該視圖是一個無障礙元素。預設情況下，所有可觸碰元素均具無障礙功能。
...
覆寫屏幕閱讀器在使用者與元素互動時朗讀的文字。預設情況下，標籤由遍歷所有子元素並合併以空格分隔的 `Text` 節點構成。
...
指定屏幕閱讀器在使用者與元素互動時應使用的語言值，需遵循 [BCP 47 規範](https://www.rfc-editor.org/info/bcp47)。
...
當無障礙標籤無法明確表達操作結果時，無障礙提示可幫助使用者理解觸發該元素動作後的預期效果。
...
表示可勾選元素的狀態。此字段可接受布林值或「mixed」字串來代表混合核取方塊。
...
指示元素將被更新，並描述用戶代理、輔助技術和用戶可以預期的實時區域更新類型。
...
布林值，指示 VoiceOver 是否應忽略接收器兄弟視圖中的元素。優先於 [`accessibilityViewIsModal`](#accessibilityviewismodal-ios) 屬性。
...
當觸碰釋放時呼叫，但若被取消（例如因滾動竊取回應者鎖定）則不呼叫。第一個函數參數是 [PressEvent](pressevent) 形式的事件。
...
當滾動視圖被停用時，此值定義觸碰可以從按鈕移出多遠的距離才會停用按鈕。一旦停用，嘗試移回按鈕會看到按鈕再次被啟用！在滾動視圖停用時來回移動數次。請傳入常數以減少記憶體配置。
