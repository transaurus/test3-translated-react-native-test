...
在以下範例中，紅色、橙色和綠色的視圖都是容器視圖的子元素，且容器設定了 `flex: 1`。紅色視圖使用 `flex: 1`，橙色視圖使用 `flex: 2`，綠色視圖使用 `flex: 3`。**1+2+3 = 6**，這意味著紅色視圖將獲得 `1/6` 的空間，橙色視圖獲得 `2/6`，綠色視圖獲得 `3/6`。
...
[`flexDirection`](layout-props#flexdirection) 控制節點子元素的排列方向，這也被稱為主軸。交叉軸則是與主軸垂直的軸，或是換行排列的方向軸。
...
- `space-evenly` 沿主軸完全均勻分布子元素。相鄰子元素間距、主軸起始端與首個子元素間距、主軸末端與最後子元素間距均完全相等。
...
[`alignItems`](layout-props#alignitems) 定義子元素在容器交叉軸上的對齊方式。其作用類似於 `justifyContent`，但作用於交叉軸而非主軸。

- `stretch`（**預設值**）拉伸子元素以匹配容器交叉軸的 `height`。

- `flex-start` 將子元素對齊至容器交叉軸起始端。
...
:::info
要使 `stretch` 生效，子元素在次軸上不能有固定尺寸。以下範例中，移除子元素的 `width: 50` 後，設置 `alignItems: stretch` 才會產生效果。
:::
...
[`alignSelf`](layout-props#alignself) 的選項與效果同 `alignItems`，但作用於單一子元素，可覆寫父容器透過 `alignItems` 設置的對齊方式。
...
[alignContent](layout-props#aligncontent) 定義多行元素在交叉軸上的分布方式，僅在使用 `flexWrap` 換行時生效。

- `flex-start`（**預設值**）將換行對齊至容器交叉軸起始端。

- `flex-end` 將換行對齊至容器交叉軸末端。

- `stretch`（_網頁版 Yoga 的預設值_）拉伸換行以匹配容器交叉軸高度。

- `center` 將換行在交叉軸上居中對齊。

- `space-between` 沿交叉軸均勻分布換行，剩餘空間分配於行間。

- `space-around` 沿交叉軸均勻分布換行，剩餘空間環繞行分配。相較於 `space-between`，使用 `space-around` 會在第一行前和最後一行後也分配空間。
...
[`flexWrap`](layout-props#flexwrap) 屬性設置在容器上，用於控制當子元素沿主軸溢出容器尺寸時的行為。預設情況下，子元素會被強制排列在單一行上（這可能會縮小元素）。如果允許換行，則會根據需要將項目沿主軸換行成多行。
...
- [`flexBasis`](layout-props#flexbasis) 是一種與軸無關的方式，用於提供項目沿主軸的預設大小。設置子元素的 `flexBasis` 類似於在父容器為 `flexDirection: row` 時設置子元素的 `width`，或在父容器為 `flexDirection: column` 時設置子元素的 `height`。項目的 `flexBasis` 是該項目的預設大小，即在任何 `flexGrow` 和 `flexShrink` 計算之前的大小。

- [`flexGrow`](layout-props#flexgrow) 描述容器內應如何沿主軸分配剩餘空間給其子元素。在佈局其子元素後，容器將根據子元素指定的 flex grow 值分配任何剩餘空間。

  `flexGrow` 接受任何大於或等於 0 的浮點值，預設值為 0。容器將根據子元素的 `flexGrow` 值加權分配剩餘空間。

- [`flexShrink`](layout-props#flexshrink) 描述當子元素的總大小在主軸上溢出容器大小時，應如何縮小子元素。`flexShrink` 與 `flexGrow` 非常相似，可以將其視為將任何溢出大小視為負剩餘空間。這兩個屬性也可以很好地協同工作，允許子元素根據需要增長和縮小。

  `flexShrink` 接受任何大於或等於 0 的浮點值，預設值為 0（在網頁上，預設值為 1）。容器將根據子元素的 `flexShrink` 值加權縮小子元素。
...
`width` 屬性指定元素內容區域的寬度。同樣，`height` 屬性指定元素內容區域的高度。
...
- `absolute` 當採用絕對定位時，元素不會參與正常的佈局流程。相反，它會獨立於其兄弟元素進行佈局。其位置由 `top`、`right`、`bottom` 和 `left` 的值決定。
