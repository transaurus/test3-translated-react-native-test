...
變換(Transforms)是能幫助您使用2D或3D轉換來修改元件外觀和位置的樣式屬性。但請注意，當您套用變換後，變換元件周圍的佈局會保持原狀，這可能導致與相鄰元件重疊。您可以透過為變換元件添加邊距(margin)、調整相鄰元件或設置容器內邊距(padding)來避免此類重疊。
...
`transform`接受一個變換物件陣列或以空格分隔的字串值。每個物件以鍵值對形式指定要變換的屬性及其值。請注意，物件不應合併使用，每個物件應只包含一組鍵值對。
...
`transformOrigin`屬性可使用一至三個值來指定偏移量：
...
`transformOrigin`也支援陣列語法，這使得與Animated API配合使用更加方便，同時避免了字串解析，效能更高。
