...
以下範例展示了不同屬性如何影響或塑造 React Native 的佈局。您可以嘗試在修改 `flexWrap` 屬性值的同時，從 UI 中添加或移除方塊。
...
`alignItems` 在交叉方向上對齊子元素。例如，如果子元素是垂直排列的，`alignItems` 控制它們在水平方向上的對齊方式。其功能類似於 CSS 中的 `align-items`（預設值為 stretch）。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/align-items。
...
`alignSelf` 控制子元素在交叉方向上的對齊方式，覆蓋父元素的 `alignItems`。其功能類似於 CSS 中的 `align-self`（預設值為 auto）。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/align-self。
...
- 在設定了 flex basis 的節點上，長寬比控制節點在交叉軸上的大小（如果未設定）
- 在具有測量函數的節點上，長寬比的作用如同測量函數測量 flex basis
- 在具有 flex grow/shrink 的節點上，長寬比控制節點在交叉軸上的大小（如果未設定）
- 長寬比會考慮最小/最大尺寸
...
`borderBottomWidth` 的功能類似於 CSS 中的 `border-bottom-width`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/border-bottom-width。
...
`borderLeftWidth` 的功能類似於 CSS 中的 `border-left-width`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/border-left-width。
...
`borderRightWidth` 的功能類似於 CSS 中的 `border-right-width`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/border-right-width。
...
`direction` 指定使用者介面的流向。預設值為 `inherit`，根節點則會根據當前語言環境決定值。詳見 https://www.yogalayout.dev/docs/styling/layout-direction。
...
`flexDirection` 控制容器中子項目的排列方向。`row` 表示從左到右排列，`column` 表示從上到下排列，其他兩個方向也可依此類推。其功能類似於 CSS 中的 `flex-direction`，但預設值為 `column`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/flex-direction。
...
`flexGrow` 描述了容器內的主軸空間應如何分配給其子項目。在佈局子項目後，容器會根據子項目指定的 `flexGrow` 值分配剩餘空間。
...
[`flexShrink`](layout-props#flexshrink) 描述了當子項目在主軸上的總大小超出容器大小時，如何沿主軸縮小子項目。`flexShrink` 與 `flexGrow` 非常相似，可以將溢出的空間視為負的剩餘空間來理解。這兩個屬性通常一起使用，以允許子項目根據需要進行擴展和收縮。
...
`flexWrap` 控制子項目在到達彈性容器末端時是否可以換行。其功能類似於 CSS 中的 `flex-wrap`（預設值為 `nowrap`）。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/flex-wrap。請注意，它不再與預設的 `alignItems: stretch` 一起使用，因此建議改用 `alignItems: flex-start` 等設定（重大變更詳情：https://github.com/facebook/react-native/releases/tag/v0.28.0）。
...
其功能類似於 CSS 中的 `height`，但在 React Native 中必須使用點數或百分比單位，不支援 em 等其他單位。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/height。
...
React Native 中的 `position` 與常規 CSS 類似，但預設值均為 `relative`，因此 `absolute` 定位始終相對於父元件。

若需以邏輯像素為單位相對於父元件定位子元件，請將子元件的定位設為 `absolute`。

若需相對於非父元件的其他元件定位子元件，請勿使用樣式實現，應透過元件樹結構處理。
...
其功能類似 CSS 中的 `width`，但在 React Native 中必須使用點數或百分比單位，不支援 em 等其他單位。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/width。
...
`zIndex` 控制元件間的疊層順序。通常不需使用 `zIndex`，元件會依文件樹順序渲染，後者覆蓋前者。僅在需要自訂動畫或模態介面等特殊行為時適用。
