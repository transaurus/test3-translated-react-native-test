...
- 若禁用 Hermes，React Native 將使用 Safari 的 JavaScript 引擎 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)。注意在 iOS 上，由於應用程式缺乏可寫入的執行記憶體，JavaScriptCore 不會使用 JIT 編譯
- 使用 Chrome 調試時，所有 JavaScript 程式碼都在 Chrome 內部運行，通過 WebSocket 與原生代碼通信。Chrome 使用 [V8](https://v8.dev/) 作為 JavaScript 引擎

雖然這些環境非常相似，但您可能會遇到一些不一致的情況。建議避免依賴特定運行環境的特性。

## JavaScript 語法轉換器

語法轉換器讓編寫程式碼更愉快，允許您使用新的 JavaScript 語法，而無需等待所有解釋器的支援。
...
完整的 React Native 啟用轉換列表可在 [@react-native/babel-preset](https://github.com/facebook/react-native/tree/main/packages/react-native-babel-preset) 找到。
...
許多標準函數在所有支援的 JavaScript 運行環境中都可用。
