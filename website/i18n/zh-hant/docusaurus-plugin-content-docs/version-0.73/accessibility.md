...
在上面的例子中，無障礙焦點僅在具有 `accessible` 屬性的父視圖上可用，而無法單獨針對「text one」和「text two」。
...
當兩個重疊的 UI 組件擁有相同父元素時，預設的無障礙焦點可能出現不可預測的行為。`importantForAccessibility` 屬性可解決此問題，它控制視圖是否觸發無障礙事件以及是否向無障礙服務報告。可設置為 `auto`、`yes`、`no` 或 `no-hide-descendants`（最後一個值會強制無障礙服務忽略該組件及其所有子元素）。
...
在上述範例中，`yellow` 佈局及其子元素對 TalkBack 和其他無障礙服務完全不可見。因此，我們可以使用同父元素的重疊視圖而不會混淆 TalkBack。

### `onAccessibilityEscape` <div class="label ios">iOS</div>

將此屬性賦予一個自訂函數，當使用者執行「退出」手勢（雙指 Z 形手勢）時會調用該函數。退出函數應在用戶界面中按層級向上移動，例如在導航層級中返回或關閉模態界面。如果選定元素沒有 `onAccessibilityEscape` 函數，系統會嘗試向上遍歷視圖層級，直到找到符合條件的視圖或發出提示音表示未找到。

### `onAccessibilityTap` <div class="label ios">iOS</div>

使用此屬性賦予一個自訂函數，當使用者在選中可訪問元素時雙擊該元素，會調用此函數。

### `onMagicTap` <div class="label ios">iOS</div>

將此屬性賦予一個自訂函數，當使用者執行「魔法點擊」手勢（雙指雙擊）時會調用該函數。魔法點擊函數應執行用戶在組件上最相關的操作。例如 iPhone 的電話應用中，魔法點擊可接聽或結束通話。如果選定元素沒有 `onMagicTap` 函數，系統會向上遍歷視圖層級直到找到符合條件的視圖。
...
- **grid** 與 ScrollView、VirtualizedList、FlatList 或 SectionList 搭配使用，表示網格。為 Android GridView 添加進出網格的語音提示。
...
- **summary** 當元素可用於在應用程式首次啟動時提供當前狀態的快速摘要時使用。
...
無障礙操作允許輔助技術以程式化方式觸發組件的操作。要支援無障礙操作，組件必須做到以下兩點：
...
操作可以是標準操作（例如點擊按鈕或調整滑桿），也可以是特定組件的自訂操作（例如刪除電子郵件）。標準和自訂操作都必須提供 `name` 欄位，但標準操作的 `label` 欄位是選填的。

當新增對標準操作的支援時，`name` 必須是以下之一：
...
`label` 欄位對於標準動作是可選的，且通常不被輔助技術使用。對於自定義動作，它是一個本地化字符串，包含要向使用者呈現的動作描述。

要處理動作請求，元件必須實現 `onAccessibilityAction` 函數。此函數的唯一參數是一個包含要執行動作名稱的事件。以下來自 RNTester 的示例展示了如何創建一個定義並處理多個自定義動作的元件。
...
有時在 UI 元件上觸發輔助功能事件很有用（例如當自定義視圖出現在螢幕上或將輔助功能焦點設置到視圖時）。原生 UIManager 模塊為此目的公開了一個方法 `sendAccessibilityEvent`。它接受兩個參數：視圖標籤和事件類型。支持的事件類型有 `typeWindowStateChanged`、`typeViewFocused` 和 `typeViewClicked`。
...
要啟用 TalkBack，請前往 Android 設備或模擬器上的「設定」應用。點擊「輔助功能」，然後選擇「TalkBack」。切換「使用服務」開關以啟用或停用它。

Android 模擬器預設未安裝 TalkBack。您可以通過 Google Play 商店在模擬器上安裝 TalkBack。請確保選擇安裝了 Google Play 商店的模擬器。這些在 Android Studio 中可用。

您可以使用音量鍵快捷方式來切換 TalkBack。要開啟音量鍵快捷方式，請前往「設定」應用，然後選擇「輔助功能」。在頂部，開啟音量鍵快捷方式。
...
要在您的 iOS 或 iPadOS 設備上啟用 VoiceOver，請前往「設定」應用，點擊「一般」，然後選擇「輔助功能」。在那裡，您會找到許多工具，包括 VoiceOver，以幫助使用者使設備更易用。要啟用 VoiceOver，請點擊「視覺」下的「VoiceOver」，然後切換頂部的開關。
