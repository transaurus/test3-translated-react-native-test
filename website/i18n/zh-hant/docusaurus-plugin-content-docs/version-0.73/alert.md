...
在 iOS 上，你可以指定任意數量的按鈕。每個按鈕可以選擇性地指定樣式或被強調，可用選項由 [AlertButtonStyle](#alertbuttonstyle-ios) 枚舉和 [AlertButton](alert#alertbutton) 上的 `isPreferred` 字段表示。
...
Android 上的警示可以通過點擊警示框外部來關閉。默認情況下此功能是禁用的，可以通過提供一個可選的 [AlertOptions](alert#alertoptions) 參數並將 `cancelable` 屬性設置為 `true` 來啟用，即<br/>`{cancelable: true}`。
