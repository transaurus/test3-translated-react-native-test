...
建議在輕量使用之外的情境中，應基於 `AsyncStorage` 進行抽象封裝而非直接操作，因其作用範圍為全域。

在 iOS 上，`AsyncStorage` 底層由原生程式碼實現，小數值會儲存於序列化字典，大數值則存於獨立檔案。Android 平台則會根據可用性選擇使用 [RocksDB](https://rocksdb.org/) 或 SQLite。

`AsyncStorage` 的 JavaScript 層為門面模式，提供清晰的 API、標準 `Error` 物件及非批次操作方法。每個 API 方法皆回傳 `Promise` 物件。
...
清除 _所有_ `AsyncStorage` 資料（含其他客戶端、函式庫等）。通常不建議直接呼叫此方法，應改用 `removeItem` 或 `multiRemove` 僅清除應用程式自身的鍵值。回傳 `Promise` 物件。
...
此方法允許您批次獲取給定鍵名陣列對應的項目。回調函數將返回找到的鍵值對陣列：
...
此方法用於批次儲存多個鍵值對。操作完成時將通過單一回調返回所有錯誤：
...
批次合併指定鍵集的現有值與新值（假設值為JSON字符串）。返回一個 `Promise` 物件。
