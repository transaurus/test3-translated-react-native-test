...
select 方法會回傳最適合當前執行平台的數值。也就是說，如果你在手機上執行，`android` 和 `ios` 鍵會優先被採用。如果這些鍵沒有指定，則會使用 `native` 鍵，接著是 `default` 鍵。
...
這將導致容器在所有平台上都有 `flex: 1`，在 Android 上有綠色背景，在 iOS 上有紅色背景，在其他平台上有藍色背景。

由於對應平台鍵的值可以是任意型別，[`select`](platform.md#select) 方法也可以用來回傳平台特定的元件，如下所示：
