...
**若您剛接觸行動開發**，最簡單的方式是使用 Expo Go。Expo 是一套圍繞 React Native 建構的工具與服務，雖然它擁有眾多[功能](https://docs.expo.dev)，目前對我們最相關的功能是它能讓您在幾分鐘內開始撰寫 React Native 應用程式。您只需要最新版的 Node.js 和一支手機或模擬器。若想在安裝任何工具前直接在網頁瀏覽器中試用 React Native，可以嘗試 [Snack](https://snack.expo.dev/)。
