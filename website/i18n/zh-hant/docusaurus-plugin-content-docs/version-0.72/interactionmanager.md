...
- `setImmediate/setTimeout()` 稍後執行代碼，請注意這可能會延遲動畫。
...
`runAfterInteractions` 接受一個普通回調函數，或是一個帶有返回 `Promise` 的 `gen` 方法的 `PromiseTask` 對象。如果提供的是 `PromiseTask`，則會完全解析（包括那些也通過 `runAfterInteractions` 安排更多任務的異步依賴項）之後，才會開始執行可能之前同步排隊的下一個任務。

默認情況下，排隊的任務會在一個 `setImmediate` 批次中一起執行。如果調用 `setDeadline` 並傳入一個正數，則任務只會在截止時間（以 js 事件循環運行時間為準）接近時執行，此時執行會通過 setTimeout 讓步，允許諸如觸控等事件開始互動，並阻止排隊任務執行，使應用更具響應性。
...
傳入正數將使用 setTimeout 在 eventLoopRunningTime 達到截止值後安排任何任務，否則所有任務會在一個 setImmediate 批次中執行（默認）。