...
建議在輕量使用之外，應基於 `AsyncStorage` 再封裝抽象層，而非直接操作全域性的 `AsyncStorage`。

在 iOS 上，`AsyncStorage` 底層由原生程式碼實現，小數值會儲存在序列化字典中，大數值則存於獨立檔案。Android 端則會根據可用性選擇使用 [RocksDB](http://rocksdb.org/) 或 SQLite。
...
根據 `key` 取得項目並在完成時觸發回調。回傳 `Promise` 物件。
...
為 `key` 設定數值並在完成時觸發回調。回傳 `Promise` 物件。
...
移除指定 `key` 的項目並在完成時觸發回調。回傳 `Promise` 物件。
...
清除 _所有_ `AsyncStorage` 資料（含所有客戶端、函式庫等）。通常不應直接呼叫此方法，請改用 `removeItem` 或 `multiRemove` 僅清除應用自身的鍵值。回傳 `Promise` 物件。
...
批次合併指定鍵集的現有值與新值（假設值為字串化JSON）。返回一個 `Promise` 物件。
