...
要渲染多欄，請使用 [`numColumns`](flatlist.md#numcolumns) 屬性。使用這種方法而非 `flexWrap` 佈局，可以避免與項目高度邏輯發生衝突。

下方提供更複雜、可選取的範例。

- 透過將 `extraData={selectedId}` 傳遞給 `FlatList`，我們確保 `FlatList` 本身會在狀態變更時重新渲染。若不設定此屬性，`FlatList` 不會知道需要重新渲染任何項目，因為它是 `PureComponent`，且屬性比較不會顯示任何變更。
- `keyExtractor` 告訴清單使用 `id` 作為 React 鍵，而非預設的 `key` 屬性。
...
這是 [`<VirtualizedList>`](virtualizedlist.md) 的便利包裝，因此繼承了其屬性（以及 [`<ScrollView>`](scrollview.md) 的屬性），未在此明確列出的屬性，連同以下注意事項：

- 當內容滾出渲染視窗時，內部狀態不會保留。請確保所有資料都儲存在項目資料或外部儲存（如 Flux、Redux 或 Relay）中。
- 這是 `PureComponent`，意味著如果 `props` 保持淺層相等，它不會重新渲染。請確保 `renderItem` 函式依賴的所有內容都作為屬性傳遞（例如 `extraData`），且在更新後不 `===`，否則你的 UI 可能不會在變更時更新。這包括 `data` 屬性和父元件狀態。
- 為了限制記憶體並實現平滑滾動，內容會異步離屏渲染。這意味著可能滾動速度快於填充速率，並暫時看到空白內容。這是可根據每個應用需求調整的取捨，我們正在幕後努力改進。
- 預設情況下，清單會尋找每個項目的 `key` 屬性並將其用作 React 鍵。或者，你可以提供自訂的 `keyExtractor` 屬性。
...
提供額外的元資料，如 `index`（如果你需要），以及更通用的 `separators.updateProps` 函式，讓你可以設定任何屬性來變更前導分隔符或尾隨分隔符的渲染，以防更常見的 `highlight` 和 `unhighlight`（設定 `highlighted: boolean` 屬性）不足以滿足你的使用情境。
...
要渲染的項目陣列（或類陣列清單）。其他資料類型可透過直接使用 [`VirtualizedList`](virtualizedlist.md) 來處理。
...
在每個項目之間渲染，但不會出現在頂部或底部。預設會提供 `highlighted` 和 `leadingItem` 屬性。`renderItem` 提供 `separators.highlight`/`unhighlight` 來更新 `highlighted` 屬性，但你也可以透過 `separators.updateProps` 新增自訂屬性。可以是 React 元件（例如 `SomeComponent`）或 React 元素（例如 `<SomeComponent />`）。
...
當清單為空時渲染。可以是 React 元件（例如 `SomeComponent`）或 React 元素（例如 `<SomeComponent />`）。
...
用於通知清單重新渲染的標記屬性（因為它實作了 `PureComponent`）。如果你的 `renderItem`、Header、Footer 等函式依賴於 `data` 屬性之外的任何內容，請將其放在這裡並視為不可變。
...
`getItemLayout` 是一個可選的優化功能，如果你事先知道項目的尺寸（高度或寬度），可以跳過動態內容的測量。`getItemLayout` 對於固定尺寸的項目非常高效，例如：
...
新增 `getItemLayout` 可以大幅提升數百個項目清單的效能。如果指定了 `ItemSeparatorComponent`，記得在偏移計算中包含分隔符的長度（高度或寬度）。
...
用於為指定索引的項目提取唯一鍵。該鍵用於快取，並作為 React 的 key 來追蹤項目重新排序。預設提取器會檢查 `item.key`，然後是 `item.id`，最後像 React 一樣回退到使用索引。
...
多列只能在 `horizontal={false}` 時渲染，並且會像 `flexWrap` 佈局一樣呈之字形排列。所有項目應具有相同高度——不支援磚石佈局。
...
視窗覆蓋百分比閾值（0-100），部分遮擋的項目需達到此百分比才會被視為「可見」。完全可見的項目始終被視為可見。數值為0表示視窗中單一像素即可使項目可見，數值為100表示項目必須完全可見或覆蓋整個視窗才會被視為可見。
...
`ViewabilityConfig`/`onViewableItemsChanged` 配對列表。當對應的 `ViewabilityConfig` 條件滿足時，會調用特定的 `onViewableItemsChanged`。詳見 `ViewabilityHelper.js` 的流程類型和進一步文檔。
