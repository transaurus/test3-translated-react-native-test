...
覆寫螢幕閱讀器在使用者與元件互動時朗讀的文字。預設情況下，標籤是透過遍歷所有子元件並將所有 `Text` 節點以空格分隔累加而成。
...
指定螢幕閱讀器在使用者與元件互動時應使用的語言，其值應符合 [BCP 47 規範](https://www.rfc-editor.org/info/bcp47)。
...
無障礙提示幫助使用者在無障礙標籤無法明確說明時，理解對無障礙元素執行操作後的結果。
...
- `'summary'` - 當元素可用於提供應用程式啟動時當前狀態的快速摘要時使用。
...
無障礙操作允許輔助技術以程式化方式觸發元件的動作。`accessibilityActions`屬性應包含動作物件列表，每個物件需包含名稱與標籤欄位。
...
表示可勾選元素的狀態。此欄位可接受布林值或「mixed」字串來代表混合狀態核取方塊。
...
指示元素將被更新，並描述用戶代理、無障礙技術和用戶可以從實時區域期待的更新類型。
