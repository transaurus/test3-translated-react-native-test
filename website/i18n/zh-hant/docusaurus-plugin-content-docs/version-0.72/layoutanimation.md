...
使用此API的常見方式是在功能組件中更新狀態鉤子之前調用它，或在類組件中調用`setState`。
...
- `duration` 以毫秒為單位
- `create`，可選配置用於動畫新視圖
- `update`，可選配置用於動畫已更新的視圖
- `delete`，可選配置用於動畫被移除的視圖
...
動畫類型的枚舉，用於[`create`](layoutanimation.md#create)方法，或用於[`configureNext`](layoutanimation.md#configurenext)的`create`/`update`/`delete`配置。（示例用法：`LayoutAnimation.Types.easeIn`）
...
佈局屬性的枚舉，用於動畫，用於[`create`](layoutanimation.md#create)方法，或用於[`configureNext`](layoutanimation.md#configurenext)的`create`/`update`/`delete`配置。（示例用法：`LayoutAnimation.Properties.opacity`）
