...
在以下範例中，紅色、橙色和綠色視圖都是容器視圖的子元素，該容器設有 `flex: 1`。紅色視圖使用 `flex: 1`，橙色視圖使用 `flex: 2`，綠色視圖使用 `flex: 3`。**1+2+3 = 6**，這意味著紅色視圖將獲得 `1/6` 的空間，橙色視圖獲得 `2/6` 的空間，綠色視圖獲得 `3/6` 的空間。
...
你可以在此處了解更多：[這裡](https://www.yogalayout.dev/docs/styling/flex-direction)。
...
佈局 [`direction`](layout-props#direction) 指定了子元素和文本在層次結構中的排列方向。佈局方向也會影響 `start` 和 `end` 所指的邊緣。預設情況下，React Native 使用 LTR（從左到右）的佈局方向。在此模式下，`start` 指的是左邊，`end` 指的是右邊。

- `LTR` (**預設值**) 文本和子元素從左到右排列。應用於元素起始位置的 margin 和 padding 會作用於左側。
- `RTL` 文本和子元素從右到左排列。應用於元素起始位置的 margin 和 padding 會作用於右側。
...
- `space-around` 在主軸上均勻分布子元素，剩餘空間分配在子元素周圍。與 `space-between` 相比，使用 `space-around` 會在首個子元素之前和最後一個子元素之後也分配空間。

- `space-evenly` 在主軸上完全均勻分布子元素。相鄰子元素之間、主軸起始端與首個子元素之間、主軸末端與最後一個子元素之間的間距完全相同。
...
[`alignItems`](layout-props#alignitems) 描述了如何沿容器的交叉軸對齊子元素。它與 `justifyContent` 非常相似，但 `alignItems` 作用於交叉軸而非主軸。
...
- `baseline` 將容器的子元素沿共同基線對齊。可以將個別子元素設置為其父元素的參考基線。
...
[`alignSelf`](layout-props#alignself) 的選項和效果與 `alignItems` 相同，但它作用於單個子元素，用於改變其在父容器中的對齊方式。`alignSelf` 會覆蓋父容器通過 `alignItems` 設置的任何選項。
...
[alignContent](layout-props#aligncontent) 定義了沿交叉軸的換行對齊方式。此屬性僅在使用 `flexWrap` 換行時有效。

- `flex-start`（**預設值**）將換行對齊到交叉軸的起始端。

- `flex-end` 將換行對齊到交叉軸的末端。
...
- `space-around` 在交叉軸上均勻分布換行，剩餘空間分配在換行周圍。與 `space-between` 相比，使用 `space-around` 會在首行之前和最後一行之後也分配空間。
...
[`flexWrap`](layout-props#flexwrap) 屬性設置在容器上，它控制當子元素在主軸方向上超出容器尺寸時的行為。預設情況下，子元素會被強制排成單行（這可能會壓縮元素）。如果允許換行，則會根據需要將項目沿主軸方向換成多行。

當換行時，可以使用 `alignContent` 來指定這些行在容器中的排列方式。了解更多[這裡](https://www.yogalayout.dev/docs/styling/flex-wrap)。
...
- [`flexBasis`](layout-props#flexbasis) 是一種與軸向無關的方式，用於提供項目在主軸上的預設尺寸。設置子元素的 `flexBasis` 類似於在父容器為 `flexDirection: row` 時設置該子元素的 `width`，或在父容器為 `flexDirection: column` 時設置子元素的 `height`。項目的 `flexBasis` 是該項目的預設尺寸，即在進行任何 `flexGrow` 和 `flexShrink` 計算之前的尺寸。

- [`flexGrow`](layout-props#flexgrow) 描述容器內的主軸空間應如何在其子元素之間分配。在佈局其子元素後，容器將根據子元素指定的 flex grow 值分配任何剩餘空間。

  `flexGrow` 接受任何大於或等於 0 的浮點數值，預設值為 0。容器將根據子元素的 `flexGrow` 值加權分配剩餘空間。

- [`flexShrink`](layout-props#flexshrink) 描述當子元素的總尺寸在主軸上超出容器尺寸時，如何沿主軸縮小子元素。`flexShrink` 與 `flexGrow` 非常相似，可以將其視為將任何溢出的尺寸視為負的剩餘空間。這兩個屬性也可以很好地協同工作，允許子元素根據需要增長和縮小。

  `flexShrink` 接受任何大於或等於 0 的浮點數值，預設值為 0（在網頁上，預設值為 1）。容器將根據子元素的 `flexShrink` 值加權縮小子元素。
...
- `auto`（**預設值**）React Native 根據元素的內容（無論是其他子元素、文字還是圖片）計算其寬度/高度。
...
- `relative` (**預設值**) 預設情況下，元素會以相對方式定位。這表示元素會依照正常的版面配置流程進行定位，然後根據 `top`、`right`、`bottom` 和 `left` 的值進行偏移。此偏移不會影響任何兄弟元素或父元素的位置。

- `absolute` 當使用絕對定位時，元素不會參與正常的版面配置流程。相反地，它會獨立於其兄弟元素進行排版。其位置是根據 `top`、`right`、`bottom` 和 `left` 的值來決定。
...
我們已經涵蓋了基礎知識，但還有許多其他樣式可能需要用於版面配置。控制版面配置的所有屬性完整列表記載於[此處](./layout-props.md)。
