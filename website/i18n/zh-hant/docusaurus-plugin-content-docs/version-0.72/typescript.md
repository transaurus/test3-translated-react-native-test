...
透過 [React Native CLI](/docs/environment-setup#creating-a-new-application) 建立的新專案或熱門模板（如 [Ignite][ignite]）預設會使用 TypeScript。

TypeScript 也可與 [Expo][expo] 搭配使用，後者提供 TypeScript 模板，或在專案中添加 `.ts` 或 `.tsx` 檔案時自動提示安裝並配置 TypeScript。
...
:::note
此指令會添加所有依賴項的最新版本。可能需要調整版本以匹配專案現有套件。您可以使用 [React Native Upgrade Helper](https://react-native-community.github.io/upgrade-helper/) 等工具查看 React Native 內建的版本。
:::
...
4. 執行 `yarn tsc` 對新的 TypeScript 檔案進行類型檢查。

## 使用 JavaScript 替代 TypeScript

React Native 新專案預設採用 TypeScript，但仍可使用 JavaScript。副檔名為 `.jsx` 的檔案會被視為 JavaScript 而非 TypeScript，且不會進行類型檢查。JavaScript 模組仍可被 TypeScript 模組導入，反之亦然。

## TypeScript 與 React Native 的協作原理

預設情況下，TypeScript 原始碼會在打包時透過 [Babel][babel] 轉換。建議僅使用 TypeScript 編譯器進行類型檢查，這是新專案中 `tsc` 的預設行為。若需將現有 TypeScript 程式碼移植到 React Native，使用 Babel 替代 TypeScript 時需注意[一兩個注意事項][babel-7-caveats]。

## React Native + TypeScript 實例展示

您可以透過 `React.Component<Props, State>` 為 React 元件的 [Props](props) 和 [State](state) 提供介面，這將在 JSX 中操作該元件時啟用類型檢查和編輯器自動完成功能。
