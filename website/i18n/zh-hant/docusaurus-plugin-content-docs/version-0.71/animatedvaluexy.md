...
用於驅動2D動畫（如平移手勢）的2D數值。API幾乎與普通[`Animated.Value`](animatedvalue)相同，但採用多路復用技術。底層包含兩個常規的`Animated.Value`。
...
停止任何正在運行的動畫或追蹤。`callback`會在停止動畫後以最終值調用，便於將狀態更新至與動畫位置同步的佈局。
