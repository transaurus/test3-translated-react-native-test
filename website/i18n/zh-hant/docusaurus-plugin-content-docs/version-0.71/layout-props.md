...
`alignSelf` 控制單個子元素在交叉軸上的對齊方式，會覆寫父元素的 `alignItems`。其功能類似 CSS 的 `align-self`（預設值為 auto）。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/align-self。
...
寬高比控制節點未定義維度的大小。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/aspect-ratio。

- 在設定了寬度/高度的節點上，寬高比控制未設定維度的大小
- 在設定了 flex basis 的節點上，若未設定交叉軸尺寸，寬高比控制節點在交叉軸的大小
- 在具有測量函式的節點上，寬高比的作用如同測量函式測量 flex basis
- 在具有 flex grow/shrink 的節點上，若未設定交叉軸尺寸，寬高比控制節點在交叉軸的大小
- 寬高比會考慮最小/最大尺寸限制
...
當方向為 `ltr` 時，`borderStartWidth` 等同於 `borderLeftWidth`；當方向為 `rtl` 時，`borderStartWidth` 則等同於 `borderRightWidth`。
...
當方向為 `ltr` 時，`end` 等同於 `right`；當方向為 `rtl` 時，`end` 則等同於 `left`。
...
當 `flex` 為正數時，會使元件具有彈性，其大小會按比例分配。例如 `flex` 設為 2 的元件會佔用兩倍於 `flex` 設為 1 的空間。`flex: <正數>` 等同於 `flexGrow: <正數>, flexShrink: 1, flexBasis: 0`。

當 `flex` 為 0 時，元件會根據 `width` 和 `height` 固定尺寸，不具彈性。

當 `flex` 為 -1 時，元件通常會根據 `width` 和 `height` 設定尺寸，但若空間不足，會縮小至 `minWidth` 和 `minHeight`。
...
[`flexShrink`](layout-props#flexshrink) 描述了當子項目的總大小在主軸上超出容器大小時，應如何沿主軸縮小子項目。`flexShrink` 與 `flexGrow` 非常相似，可以將任何溢出的空間視為負的剩餘空間來理解。這兩個屬性可以很好地配合使用，讓子項目能夠根據需要伸展或收縮。
...
`flexWrap` 控制子項目在到達彈性容器末端後是否可以換行。其功能類似於 CSS 中的 `flex-wrap`（預設值為 `nowrap`）。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/flex-wrap 以獲取更多細節。請注意，它不再與預設的 `alignItems: stretch` 一起使用，因此您可能需要改用 `alignItems: flex-start`（重大變更詳情：https://github.com/facebook/react-native/releases/tag/v0.28.0）。
...
其功能類似於 CSS 中的 `height`，但在 React Native 中必須使用點數或百分比單位，不支援 em 等其他單位。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/height 以獲取更多細節。
...
`paddingTop` 的功能類似 CSS 中的 `padding-top`。詳情請參閱 https://developer.mozilla.org/en-US/docs/Web/CSS/padding-top。
...
React Native 中的 `position` 與常規 CSS 類似，但預設所有元素均設為 `relative`，因此 `absolute` 定位始終相對於父元素。

若需以邏輯像素的具體數值相對於父元素定位子元素，請將子元素的定位設為 `absolute`。
...
`rowGap` 的功能類似 CSS 中的 `row-gap`。React Native 僅支援像素單位。詳情請參閱 https://developer.mozilla.org/en-US/docs/Web/CSS/row-gap。
...
其功能類似 CSS 中的 `width`，但在 React Native 中必須使用點數或百分比單位，不支援 em 等其他單位。詳情請參閱 https://developer.mozilla.org/en-US/docs/Web/CSS/width。
...
`zIndex` 控制元件間的疊層順序。通常不需使用 `zIndex`，元件會依文件樹中的順序渲染，後面的元件會覆蓋前面的元件。但在動畫或自訂模態介面等需要改變此行為時，`zIndex` 會很有用。
