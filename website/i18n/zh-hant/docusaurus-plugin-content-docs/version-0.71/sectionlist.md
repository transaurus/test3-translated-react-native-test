...
- 為了限制記憶體使用並實現平滑滾動，內容會異步渲染到螢幕外。這意味著可能會滾動得比填充速度更快，並暫時看到空白內容。這是一個可以根據每個應用需求調整的權衡，我們正在幕後努力改進這一點。
...
初始批次中要渲染的項目數量。這應該足以填滿螢幕，但不需要過多。請注意，這些項目永遠不會因為視窗化渲染而被卸載，以提升滾動至頂部操作的使用體驗。
...
用於為指定索引的項目提取唯一鍵。鍵用於快取和作為 React 鍵來追蹤項目重新排序。預設提取器檢查 `item.key`，然後回退到使用索引，就像 React 所做的那樣。請注意，這為每個項目設置了鍵，但每個整體部分仍需要自己的鍵。
...
列表底部邊緣距離內容末尾多遠（以列表可見長度為單位）時觸發 `onEndReached` 回調。因此，值為 0.5 時，當內容末尾在列表可見長度的一半以內時，會觸發 `onEndReached`。
...
如果提供，將添加標準的 RefreshControl 以實現「下拉刷新」功能。確保同時正確設置 `refreshing` 屬性。若要從頂部偏移 RefreshControl（例如 100 點），請使用 `progressViewOffset={100}`。
...
在每個區段的頂部渲染。在 iOS 上預設會黏附在 `ScrollView` 的頂部。請參閱 `stickySectionHeadersEnabled`。
...
在每個區段的頂部和底部渲染（注意這與 `ItemSeparatorComponent` 不同，後者僅在項目之間渲染）。這些用於將區段與上方和下方的標頭分隔開，通常具有與 `ItemSeparatorComponent` 相同的高亮響應。同時接收 `highlighted`、`[leading/trailing][Item/Section]` 以及來自 `separators.updateProps` 的任何自定義屬性。
...
讓區段標頭黏附在螢幕頂部，直到下一個標頭將其推離。預設僅在 iOS 上啟用，因為這是該平台的標準。
...
通知列表已發生互動，這應觸發可見性計算，例如當 `waitForInteractions` 為 true 且用戶尚未滾動時。通常由點擊項目或導航操作調用。
...
滾動到指定 `sectionIndex` 和 `itemIndex`（在區段內）的項目，並將其定位在可見區域中，其中 `viewPosition` 為 0 時將其置於頂部（可能被黏性標頭覆蓋），1 時置於底部，0.5 時置於中間。
...
- 'viewOffset' (number) — 固定像素數以偏移最終目標位置，例如用於補償黏性標頭。
