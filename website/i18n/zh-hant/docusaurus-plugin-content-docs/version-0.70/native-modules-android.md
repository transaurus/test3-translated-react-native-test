...
目前我們不建議這樣做，因為同步呼叫方法可能會嚴重影響效能，並在您的原生模組中引入與線程相關的錯誤。此外，請注意，如果您選擇啟用 `isBlockingSynchronousMethod`，您的應用程式將無法再使用 Google Chrome 調試器。這是因為同步方法要求 JS VM 與應用程式共享記憶體。對於 Google Chrome 調試器，React Native 運行在 Google Chrome 內的 JS VM 中，並通過 WebSockets 與移動設備進行異步通信。
...
要將您的原生模組添加到 `ReactPackage`，首先在 `android/app/src/main/java/com/您的應用名稱/` 文件夾中創建一個新的 Java/Kotlin 類（`MyAppPackage.java` 或 `MyAppPackage.kt`），並實現 `ReactPackage`：
...
至此，您已為 Android 原生模組完成基礎架構設置。請透過 JavaScript 存取該原生模組並呼叫其導出方法來進行測試。

在應用程式中找到您想呼叫原生模組 `createCalendarEvent()` 方法的位置。以下是一個可加入應用程式的元件範例 `NewModuleButton`，您可以在該按鈕的 `onPress()` 函數中觸發原生模組。
...
最後一步是重新建置 React Native 應用程式，以確保最新的原生程式碼（包含您的新原生模組）生效。在 React Native 應用程式根目錄的命令列中執行：
...
當您遵循本指南迭代開發原生模組時，需透過原生建置才能使 JavaScript 端取得最新變更。這是因為您編寫的程式碼屬於應用程式的原生部分。雖然 React Native 的 Metro 打包工具能監聽 JavaScript 變更並即時重建，但對原生程式碼無效。若要測試最新的原生修改，必須使用 `npx react-native run-android` 指令重新建置。

### 重點回顧 ✨

現在您應能在應用程式中呼叫原生模組的 `createCalendarEvent()` 方法。本範例透過點擊 `NewModuleButton` 觸發。您可檢視 `createCalendarEvent()` 方法中設定的日誌來確認執行結果。請參照[這些步驟](https://developer.android.com/studio/debug/am-logcat.html)查看應用程式的 ADB 日誌。搜尋您設定的 `Log.d` 訊息（本範例為「Create event called with name: testName and location: testLocation」），每次呼叫原生模組方法時都應看到該日誌訊息。
...
透過 `NativeModules` 提取原生模組的現行導入方式較為笨拙。

為避免模組使用者每次存取時重複此操作，可為原生模組建立 JavaScript 封裝層。建立名為 `CalendarModule.js` 的新 JavaScript 檔案，內容如下：
...
此 JavaScript 檔案也是添加 JavaScript 端功能的理想位置。例如若使用 TypeScript 等型別系統，可在此為原生模組添加型別註解。雖然 React Native 尚未支援原生到 JS 的型別安全，但所有 JS 程式碼都將具備型別安全性。此做法也能為未來遷移至型別安全的原生模組預作準備。以下是為 CalendarModule 添加型別安全的範例：
...
當原生模組方法在 JavaScript 中被調用時，React Native 會將參數從 JS 物件轉換為對應的 Java/Kotlin 物件。舉例來說，若您的 Java 原生模組方法接收一個 double 類型，在 JS 中您需要用數字來調用該方法。React Native 會自動處理轉換。以下是原生模組方法支援的參數類型及其對應的 JavaScript 等效類型清單。
...
對於未列出的參數類型，您需自行處理轉換。例如在 Android 中，`Date` 轉換並未內建支援。您可以在原生方法中自行處理轉換為 `Date` 類型，如下所示：
...
接著可透過在 JS 中調用原生模組的 `getConstants` 來存取該常數：
...
> 請注意目前常數僅在初始化時導出，若在運行時變更 `getConstants` 的值，不會影響 JavaScript 環境。此行為將在 TurboModules 中改變。屆時 `getConstants()` 將成為常規原生模組方法，每次調用都會觸達原生端。

### 回調函數

原生模組還支援一種特殊參數：回調函數。回調用於在非同步方法中將資料從 Java/Kotlin 傳遞至 JavaScript，也可用於從原生端非同步執行 JavaScript。

要建立帶回調的原生模組方法，需先導入 `Callback` 介面，然後在方法參數中添加 `Callback` 類型參數。回調參數有幾項 TurboModules 即將解除的限制：首先，函數參數中最多只能有兩個回調——successCallback 和 failureCallback。此外，若原生模組方法調用的最後一個參數是函數，它會被視為 successCallback；倒數第二個參數若是函數，則被視為 failureCallback。
...
您可在 Java/Kotlin 方法中調用回調，傳遞任何想送至 JavaScript 的資料。請注意只能傳遞可序列化資料，若需回傳原生物件可使用 `WriteableMaps`，集合則用 `WritableArrays`。另需強調回調不會在原生函數完成後立即觸發。以下範例將先前調用所建立事件的 ID 傳遞給回調：
...
需特別注意：原生模組方法只能調用一個回調一次。這意味著您只能調用成功回調或失敗回調其一，且每個回調最多觸發一次。但原生模組可儲存回調並延後觸發。

回調的錯誤處理有兩種方式：第一是遵循 Node 慣例，將回調的第一個參數視為錯誤物件。
...
更多細節，您可以在[這裡](https://github.com/facebook/react-native/blob/0.70-stable/ReactAndroid/src/main/java/com/facebook/react/bridge/Promise.java#L1)找到 `Promise.java` 介面。如果未提供 `userInfo`，React Native 會將其設為 null。對於其餘參數，React Native 會使用預設值。`message` 參數提供了錯誤呼叫堆疊頂部顯示的錯誤 `message`。以下是從 Java/Kotlin 中的 reject 呼叫在 JavaScript 中顯示的錯誤訊息範例。
...
然後，JavaScript 模組可以通過在 [NativeEventEmitter](https://github.com/facebook/react-native/blob/0.70-stable/Libraries/EventEmitter/NativeEventEmitter.js) 類上 `addListener` 來註冊接收事件。
