...
> **已棄用。** 請改用[社群套件](https://reactnative.directory/?search=storage)中的替代方案。

`AsyncStorage` 是一個未加密、非同步、持久化且全域的鍵值儲存系統，應取代 LocalStorage 使用。

建議在輕量使用之外，應基於 `AsyncStorage` 進行抽象封裝而非直接使用，因其運作於全域層級。

在 iOS 上，`AsyncStorage` 底層由原生程式碼實現，小數值會儲存於序列化字典，大數值則存於獨立檔案。Android 端會根據可用性使用 [RocksDB](http://rocksdb.org/) 或 SQLite。

`AsyncStorage` 的 JavaScript 層提供清晰 API、真實 `Error` 物件及非批次方法，所有 API 方法均返回 `Promise` 物件。
...
為 `key` 設定值並在完成時觸發回調。返回 `Promise` 物件。
...
取得應用程式 _所有_ 已知金鑰（含其他呼叫者、函式庫等）。返回 `Promise` 物件。
