...
**若您是行動開發的新手**，最簡單的方式是使用 Expo Go 開始。Expo 是一套圍繞 React Native 建構的工具與服務，雖然它具備許多[功能](https://docs.expo.dev)，目前對我們最相關的功能是它能讓您在幾分鐘內開始撰寫 React Native 應用程式。您只需要最新版本的 Node.js 和一支手機或模擬器。若您想在安裝任何工具前，先透過網頁瀏覽器試用 React Native，可以嘗試 [Snack](https://snack.expo.dev/)。

**若您已熟悉行動開發**，可能會想使用 React Native CLI。它需要 Xcode 或 Android Studio 才能開始。若您已安裝其中一項工具，應該能在幾分鐘內開始運作。若尚未安裝，預期需花費約一小時進行安裝與設定。
