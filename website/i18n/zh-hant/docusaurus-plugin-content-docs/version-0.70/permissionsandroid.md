...
如果使用者先前關閉了您請求的權限，作業系統會建議您的應用程式說明需要該權限的理由。可選的 `rationale` 參數僅在必要時顯示對話框提示——否則將出現標準的權限請求提示。
...
若提供 `rationale` 參數，此函式會向作業系統確認是否需要顯示對話框說明為何需要該權限 (https://developer.android.com/training/permissions/requesting.html#explain)，然後顯示系統權限對話框。
