...
在 iOS 平台自訂背景色。此背景色會在開關值為 `false` 或開關被禁用時顯示（此時開關呈半透明狀）。
...
_iOS 備註_：當開關值為 `false` 時，軌道會向邊界收縮。若要調整收縮後露出的背景色，請使用 [`ios_backgroundColor`](switch.md#ios_backgroundColor) 屬性。
