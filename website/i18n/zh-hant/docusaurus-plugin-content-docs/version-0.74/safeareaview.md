...
使用時，請將您的頂層視圖用一個套用了 `flex: 1` 樣式的 `SafeAreaView` 包裹起來。您可能還需要使用與應用程式設計相匹配的背景顏色。
...
> 由於內邊距用於實現該元件的行為，套用到 `SafeAreaView` 的樣式中的內邊距規則將被忽略，並可能根據平台導致不同的結果。詳情請參閱 [#22211](https://github.com/facebook/react-native/issues/22211)。