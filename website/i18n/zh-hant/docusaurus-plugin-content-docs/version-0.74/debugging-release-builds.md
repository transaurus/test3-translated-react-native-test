...
在上述堆疊追蹤中，類似 `p@1:132161` 的條目代表經過最小化的函式名稱與位元組碼偏移量。要除錯這些呼叫，我們需將其轉換為檔案名稱、行號與函式名稱，例如 `AwesomeProject/App.js:54:initializeMap`。此過程稱為**符號化**。
...
- 建置過程可能產生多份原始碼對應檔，請確保使用範例所示路徑的對應檔。
- 使用的原始碼對應檔必須與當機應用程式的提交版本完全匹配，原始碼的微小變動可能導致偏移量大幅差異。
