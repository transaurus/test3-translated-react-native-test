...
- 若禁用 Hermes，React Native 將採用 Safari 的底層引擎 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)。請注意在 iOS 上，由於應用程式缺乏可寫入的執行記憶體，JavaScriptCore 無法使用 JIT 編譯
- 使用 Chrome 調試時，所有 JavaScript 程式碼都在 Chrome 內部執行，並通過 WebSocket 與原生代碼通信。此時採用的是 Chrome 的 [V8](https://v8.dev/) 引擎

雖然這些環境非常相似，但仍可能遇到某些不一致的情況。建議避免依賴特定執行環境的特性。

## JavaScript 語法轉換器

語法轉換器讓編寫程式碼更愉快，允許您使用新的 JavaScript 語法而無需等待所有直譯器的支援。
...
許多標準函式在所有支援的 JavaScript 執行環境中都可使用。
