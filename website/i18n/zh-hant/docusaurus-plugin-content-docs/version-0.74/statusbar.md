...
控制應用程式狀態列的元件。狀態列通常是螢幕頂部的區域，顯示當前時間、Wi-Fi和行動網路資訊、電量等級和其他狀態圖示。
...
對於不適合使用元件的情況，該元件也提供了以靜態函數形式暴露的命令式API。但不建議同時使用靜態API和元件來控制相同屬性，因為靜態API設置的值會在下次渲染時被元件設置的值覆蓋。
...
是否在狀態列屬性變更時啟用過渡動畫。支援`backgroundColor`、`barStyle`和`hidden`屬性。
...
狀態列是否為半透明。當設為`true`時，應用程式會繪製在狀態列下方。這在使用半透明狀態列顏色時特別有用。
...
將StatusBar條目推入堆疊。返回值應在完成時傳遞給`popStackEntry`。
