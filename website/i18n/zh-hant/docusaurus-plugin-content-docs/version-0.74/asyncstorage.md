...
建議在輕量使用之外的情境中，採用基於 `AsyncStorage` 的抽象層而非直接操作，因其運作於全域層級。

在 iOS 上，`AsyncStorage` 由原生程式碼實現，小數值會儲存於序列化字典，較大數值則存於獨立檔案。Android 端會根據可用性選擇使用 [RocksDB](https://rocksdb.org/) 或 SQLite。

`AsyncStorage` 的 JavaScript 程式碼為一門面模式，提供清晰的 JavaScript API、真實的 `Error` 物件及非多重操作函式。每個 API 方法皆回傳 `Promise` 物件。
...
根據 `key` 取得項目並於完成時觸發回呼。回傳 `Promise` 物件。
...
為 `key` 設定數值並於完成時觸發回呼。回傳 `Promise` 物件。
...
移除 `key` 對應項目並於完成時觸發回呼。回傳 `Promise` 物件。
...
合併既有 `key` 數值與輸入值（假設兩者皆為 JSON 字串）。回傳 `Promise` 物件。
...
清除 _所有_ `AsyncStorage` 資料（含所有客戶端、函式庫等）。通常不建議直接呼叫此方法，應改用 `removeItem` 或 `multiRemove` 僅清除應用自身的鍵值。回傳 `Promise` 物件。
...
取得應用已知的 _所有_ 鍵值（含所有呼叫者、函式庫等）。回傳 `Promise` 物件。
...
使用此方法批次儲存多個鍵值對。操作完成時將收到單一回調函數包含任何錯誤：
