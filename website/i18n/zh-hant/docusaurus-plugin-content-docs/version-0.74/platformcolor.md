...
由於原生色彩可能受主題或高對比度設定影響，此平台特定邏輯也會在您的元件內部自動適應。
...
提供給 `PlatformColor` 函數的字符串值必須與應用程式運行時所在原生平台的字符串完全匹配。為避免運行時錯誤，應將函數包裹在平台檢查邏輯中（如 `Platform.OS === 'platform'` 或 `Platform.select()`），如上例所示。
