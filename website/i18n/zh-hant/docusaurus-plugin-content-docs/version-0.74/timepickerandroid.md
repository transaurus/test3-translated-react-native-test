...
回傳一個 Promise，當用戶選擇時間時會解析為包含 `action`、`hour` (0-23)、`minute` (0-59) 的物件。若用戶關閉對話框，Promise 仍會解析，但 `action` 會是 `TimePickerAndroid.dismissedAction`，其他鍵值則為 undefined。**務必**在讀取值之前檢查 `action`。
