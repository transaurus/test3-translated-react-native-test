...
本地函式庫是指包含視圖或模組且位於應用程式本地的函式庫，這些函式庫並未發佈到註冊表中。與傳統的視圖和模組設置不同，本地函式庫與應用程式的原生代碼是解耦的。

本地函式庫創建於 `android/` 和 `ios/` 文件夾之外，並利用自動連結（autolinking）來與應用程式整合。包含本地函式庫的結構可能如下所示：
...
由於本地函式庫的代碼位於 `android/` 和 `ios/` 文件夾之外，這使得未來升級 React Native 版本、複製到其他項目等操作更加容易。
...
默認情況下，生成的函式庫會自動連結到項目中：使用 Yarn 時會使用 `link:` 協議，而使用 npm 時則會使用 `file:` 協議：
...
這會在 `node_modules` 下創建一個指向函式庫的符號連結，從而實現自動連結的功能。
