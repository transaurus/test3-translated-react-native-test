...
React Native 提供了多種內建的[核心元件](intro-react-native-components)，可直接用於您的應用程式。您可以在左側邊欄（或螢幕上方的選單，如果您使用的是窄螢幕）中找到所有元件。如果不確定從何開始，請參考以下分類：
...
這些元件可能對特定應用程式有所幫助。如需完整的元件與 API 列表，請查看左側邊欄（或螢幕上方的選單，如果您使用的是窄螢幕）。
