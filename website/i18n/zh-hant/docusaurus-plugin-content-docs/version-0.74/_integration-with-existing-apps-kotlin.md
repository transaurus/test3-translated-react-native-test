...
如果您的應用程式目標是 Android `API 等級 23` 或更高，請確保您為開發版本啟用了 `android.permission.SYSTEM_ALERT_WINDOW` 權限。您可以使用 `Settings.canDrawOverlays(this)` 來檢查這一點。這在開發版本中是必需的，因為 React Native 開發錯誤必須顯示在所有其他視窗之上。由於 API 等級 23 (Android M) 中引入的新權限系統，用戶需要批准它。這可以通過將以下程式碼新增到您的 Activity 的 `onCreate()` 方法中來實現。
...
最後，必須覆寫 `onActivityResult()` 方法（如下面的程式碼所示）以處理權限接受或拒絕的情況，以保持一致的用戶體驗。此外，對於使用 `startActivityForResult` 的整合原生模組，我們需要將結果傳遞給我們的 `ReactInstanceManager` 實例的 `onActivityResult` 方法。
...
> 如果您使用的是 React Native 的入門套件，請將 "HelloWorld" 字串替換為您的 index.js 檔案中的字串（它是 `AppRegistry.registerComponent()` 方法的第一個參數）。
...
> 單個 `ReactInstanceManager` 可被多個 Activity 或 Fragment 共享。建議建立自定義的 `ReactFragment` 或 `ReactActivity`，並通過單例模式持有 `ReactInstanceManager`。當需要連接生命週期時（例如將 `ReactInstanceManager` 綁定到這些 Activity/Fragment），使用該單例提供的實例。
...
這允許 JavaScript 控制硬體返回鍵的行為（例如實現導航）。若 JavaScript 未處理返回事件，則會調用 `invokeDefaultOnBackPressed` 方法，默認情況下會結束當前 `Activity`。

最後需連接開發者選單。默認通過搖動設備觸發，但在模擬器中可改為通過硬體選單鍵觸發（Android Studio 模擬器使用 <kbd>Ctrl</kbd> + <kbd>M</kbd>）：
...
完成基本整合步驟後，需啟動 [Metro 打包工具][metro] 來構建 `index.bundle` 並運行本地伺服器提供該文件。

##### 1. 運行打包工具

首先在 React Native 專案根目錄執行以下命令啟動開發伺服器：
...
像往常一樣構建並運行您的 Android 應用程式。

當進入整合 React 的 Activity 時，應會從開發伺服器加載 JavaScript 代碼並顯示：

![螢幕截圖](/docs/assets/EmbeddedAppAndroid.png)

### 在 Android Studio 中建立正式版構建

您可以使用 Android Studio 建立正式版構建，過程與原有原生 Android 應用相同。

若按照前述方式使用 React Native Gradle Plugin，從 Android Studio 運行應用時應能正常工作。

若未使用該插件，則每次正式構建前需額外執行以下命令來生成 React Native 套件（該套件將被包含在原生應用中）：
...
現在像往常一樣通過 Android Studio 建立正式版構建即可。

### 後續步驟

您可繼續常規開發流程，詳見 [除錯指南](debugging) 和 [部署指南](running-on-device) 以深入瞭解 React Native 開發。
